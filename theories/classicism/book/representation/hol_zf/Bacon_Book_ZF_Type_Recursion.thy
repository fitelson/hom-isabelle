...
primrec full_ZF_h :: "otype \<Rightarrow> 'c book_C_world \<Rightarrow> 'c book_named_term set \<Rightarrow> ZF" where
  "full_ZF_h Ind = (\<lambda>w X. book_ZF_countable_set_code X)"
...
theorem full_ZF_D_elements:
  "explode (full_ZF_D \<sigma> w) = full_ZF_h \<sigma> w ` book_C_identity_domain (fst w) G (snd w) \<sigma>"
  by (simp only: full_ZF_D_def full_ZF_domain_elements)

lemma full_ZF_h_type:
  "X \<in> book_C_identity_domain (fst w) G (snd w) \<sigma> \<Longrightarrow> full_ZF_h \<sigma> w X \<in> explode (full_ZF_D \<sigma> w)"
  by (simp only: full_ZF_D_elements; rule imageI; assumption)

lemma full_ZF_j_type:
  "a \<in> explode (full_ZF_D \<sigma> w) \<Longrightarrow> full_ZF_j \<sigma> w a \<in> book_C_identity_domain (fst w) G (snd w) \<sigma>"
  by (simp only: full_ZF_j_def full_ZF_D_def; rule full_ZF_inverse_type; assumption)
...
proof -
  have image_member: "a \<in> full_ZF_h \<sigma> w ` book_C_identity_domain (fst w) G (snd w) \<sigma>"
    using member by (simp only: full_ZF_D_elements)
...
text \<open>
  ...
  Initial range/typing and h∘j laws are checked. Injectivity, j∘h, the
