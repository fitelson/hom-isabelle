...
proof -
  ...
  show ?thesis unfolding full_ZF_i_def
    by (simp only: full_ZF_h_proposition_restriction[OF ww vw wv inverse] full_ZF_hj[OF worlds_admitted[OF ww] member])
qed
...
proof -
  obtain X where shape: "p = full_ZF_h Prop w X"
    using member unfolding full_ZF_D_elements[OF admitted] by blast
...
theorem full_ZF_individual_domain:
  ...
  shows "full_ZF_D Ind w = book_ZF_term_class_domain G w Ind"
  by (rule injD[OF inj_explode]; simp only: full_ZF_D_elements[OF admitted] full_ZF_h.simps book_ZF_term_class_domain_elements)
