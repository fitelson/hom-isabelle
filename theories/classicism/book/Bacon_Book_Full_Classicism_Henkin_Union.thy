theory Bacon_Book_Full_Classicism_Henkin_Union
  imports Bacon_Book_Full_Classicism_Henkin_Stages
    Bacon_Book_Environment_Development.Bacon_Book_Henkin_Union
begin

lemma book_full_C_consistent_directed_Union:
  assumes nonempty: "\<C> \<noteq> {}"
    ...
    and consistent: "\<And>U. U \<in> \<C> \<Longrightarrow> book_full_C_theory_consistent \<Sigma> G U"
  shows "book_full_C_theory_consistent \<Sigma> G (\<Union>\<C>)"
proof (rule iffD2[OF book_full_C_theory_consistent_finite_character], intro allI impI)
  fix F
  ...
  show "book_full_C_theory_consistent \<Sigma> G F" by (rule book_full_C_consistent_subset[OF consistent[OF member] covered])
qed

theorem book_full_C_henkin_full_premises_consistent:
  ...
  shows "book_full_C_theory_consistent (book_henkin_full_signature \<Sigma> G) G (book_henkin_full_premises \<Sigma> G S)"
proof -
  let ?P = "book_henkin_premises \<Sigma> G S"
  let ?F = "range ?P"
  have nonempty: "?F \<noteq> {}" by simp
  have stages: "book_full_C_theory_consistent (book_henkin_full_signature \<Sigma> G) G (?P n)" for n
  proof -
    have previous: "book_full_C_theory_consistent (book_henkin_signature \<Sigma> G n) G (?P n)"
      by (rule book_full_C_henkin_premises_consistent[OF rich language consistent])
    ...
    show ?thesis by (rule book_full_C_theory_consistency_signature_preservation[OF rich previous stage_language])
  qed
  have all_consistent: "book_full_C_theory_consistent (book_henkin_full_signature \<Sigma> G) G U" if "U \<in> ?F" for U
  ...
  qed
  have union: "book_full_C_theory_consistent (book_henkin_full_signature \<Sigma> G) G (\<Union>?F)"
    by (rule book_full_C_consistent_directed_Union[OF nonempty directed all_consistent])
