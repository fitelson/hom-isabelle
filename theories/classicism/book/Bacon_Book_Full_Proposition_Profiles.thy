theory Bacon_Book_Full_Proposition_Profiles
...
definition book_full_C_proposition_profile where
  "book_full_C_proposition_profile \<Sigma> B G actual w P =
    {v \<in> book_full_C_rooted_worlds \<Sigma> B G actual. book_C_canonical_le G w v \<and> P \<in> snd v}"
...
theorem proposition_profiles_eq_iff_identity:
  ...
  shows "book_full_C_proposition_profile \<Sigma> B G actual w P = book_full_C_proposition_profile \<Sigma> B G actual w Q \<longleftrightarrow>
...
proof -
  ...
  show ?thesis
  proof
    assume equal: "book_full_C_proposition_profile \<Sigma> B G actual w P = book_full_C_proposition_profile \<Sigma> B G actual w Q"
    ...
    proof (intro ballI impI)
      ...
      interpret V: book_C_identity_world "fst v" G "snd v"
        by (rule book_full_C_world_identity_algebra[OF rich vv])
      have pv: "P \<in> book_closed_terms (fst v) G Prop" by (rule book_full_C_closed_terms_future[OF rich ww vv access pm])
      have qv: "Q \<in> book_closed_terms (fst v) G Prop" by (rule book_full_C_closed_terms_future[OF rich ww vv access qm])
      have same: "P \<in> snd v \<longleftrightarrow> Q \<in> snd v"
        using equal vw access unfolding book_full_C_proposition_profile_def by blast
    ...
    show "book_leibniz G Prop P Q \<in> snd w"
      by (rule book_full_C_world_boxed_propositional_equivalence[OF rich ww pl ql pc qc necessary])
  next
    ...
    proof -
      ...
      have future_identity: "book_leibniz G Prop P Q \<in> snd v"
        by (rule book_full_C_canonical_identity_persistence[OF rich ww access pl ql pc qc identity])
      show ?thesis by (rule V.proposition_identity_membership[OF pv qv future_identity])
    qed
    show "book_full_C_proposition_profile \<Sigma> B G actual w P = book_full_C_proposition_profile \<Sigma> B G actual w Q"
      unfolding book_full_C_proposition_profile_def using same by blast
  qed
qed
