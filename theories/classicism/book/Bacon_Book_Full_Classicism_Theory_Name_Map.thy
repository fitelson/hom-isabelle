theory Bacon_Book_Full_Classicism_Theory_Name_Map
  imports Bacon_Book_Full_Classicism_Name_Map Bacon_Book_Full_Classicism_Theory_Consistency
begin

section \<open>Type-indexed transport of full C with additional assumptions\<close>

theorem book_full_C_theory_typed_name_map:
  ...
  shows "book_full_C_theory_derivable \<Omega> G (book_typed_name_map \<rho> ` S) (book_typed_name_map \<rho> A)"
proof -
  let ?C = "{B. book_full_C_proves \<Sigma> G B}"
  let ?D = "{B. book_full_C_proves \<Omega> G B}"
  ...
  proof (rule book_theory_derivable_cut[OF mapped])
    ...
    proof (cases "book_full_C_proves \<Sigma> G B")
      case True
      have target_C: "book_full_C_proves \<Omega> G (book_typed_name_map \<rho> B)" by (rule book_full_C_typed_name_map[OF rich True maps])
      have target_type: "book_theory_formula \<Omega> G (book_typed_name_map \<rho> B)" by (rule book_full_C_proves_language[OF rich target_C])
