theory Bacon_Book_Classicism_Theory_Retraction
  imports Bacon_Book_Classicism_Signature_Conservativity Bacon_Book_Classicism_Theory_Rules
begin

section \<open>One fresh retraction for the finitely used C background\<close>

theorem book_C_theory_retraction_fresh:
  ...
  shows "\<exists>v. (\<forall>\<tau>. G (v \<tau>) = \<tau>) \<and> (\<forall>\<tau>. v \<tau> \<notin> Avoid) \<and>
    book_C_theory_derivable \<Omega> G S (named_retract \<Omega> v A)"
proof -
  ...
  let ?Used = "U \<inter> ?C"
  let ?N = "\<lambda>B. SOME N. book_C_retraction_support \<Omega> G B N"
  have support_C: "book_C_retraction_support \<Omega> G B (?N B)" if member: "B \<in> ?Used" for B
  proof -
    have theorem_C: "book_C_proves \<Sigma> G B" using member by simp
    show ?thesis by (rule someI_ex, rule book_C_retraction_support_exists[OF rich theorem_C])
  qed
  have finite_C: "finite ?Used" using finite_U by simp
  have finite_N: "finite (?N B)" if "B \<in> ?Used" for B
    using support_C[OF that] unfolding book_C_retraction_support_def by blast
  ...
  have lifted: "book_theory_derivable \<Omega> G ({B. book_C_proves \<Omega> G B} \<union> S) (named_retract \<Omega> ?v A)"
  proof (rule book_theory_derivable_cut[OF retracted])
    ...
    show "book_theory_derivable \<Omega> G ({B. book_C_proves \<Omega> G B} \<union> S) Q"
    proof (cases "book_C_proves \<Sigma> G B")
      case True
      have used: "B \<in> ?Used" using member True by simp
      have target_C: "book_C_proves \<Omega> G (named_retract \<Omega> ?v B)"
        by (rule book_C_retraction_apply[OF support_C[OF used] stock fresh_C[OF used]])
      have target_type: "book_theory_formula \<Omega> G (named_retract \<Omega> ?v B)"
        by (rule book_C_proves_language[OF rich target_C])
      have target_member: "named_retract \<Omega> ?v B \<in> {P. book_C_proves \<Omega> G P} \<union> S"
    ...
    qed
  qed
  have result: "book_C_theory_derivable \<Omega> G S (named_retract \<Omega> ?v A)"
    using lifted unfolding book_C_theory_derivable_def .
...
theorem book_C_theory_foreign_constants_eliminate:
  assumes rich: "sg_rich G" and derivation: "book_C_theory_derivable \<Sigma> G S A"
    and old_premises: "\<And>B. B \<in> S \<Longrightarrow> book_theory_formula \<Omega> G B"
    and old_conclusion: "named_in_signature \<Omega> A"
  shows "book_C_theory_derivable \<Omega> G S A"
proof -
  have finite: "finite ({} :: nat set)" by simp
  obtain v where retracted: "book_C_theory_derivable \<Omega> G S (named_retract \<Omega> v A)"
    using book_C_theory_retraction_fresh[OF rich derivation old_premises finite] by blast
  show ?thesis using retracted by (simp only: named_retract_fixed[OF old_conclusion])
qed

corollary book_C_theory_consistency_signature_preservation:
  assumes rich: "sg_rich G" and consistent: "book_C_theory_consistent \<Omega> G S"
    and old_premises: "\<And>B. B \<in> S \<Longrightarrow> book_theory_formula \<Omega> G B"
...
proof (unfold book_C_theory_consistent_def, rule notI)
  ...
  have reflected: "book_C_theory_derivable \<Omega> G S (book_bottom G)"
    by (rule book_C_theory_foreign_constants_eliminate[OF rich contradiction old_premises old_bottom])
  show False using consistent reflected unfolding book_C_theory_consistent_def by contradiction
qed
