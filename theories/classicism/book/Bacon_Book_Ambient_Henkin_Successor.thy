theory Bacon_Book_Ambient_Henkin_Successor
  imports Bacon_Book_Ambient_Henkin_Extension Bacon_Book_Classicism_Closed_Accessibility
begin
...
theorem book_C_ambient_henkin_successor_exists:
  ...
  shows "\<exists>v. book_C_closed_maximal_extension (book_ambient_henkin_signature G) G {} v \<and>
    book_C_theory_consistent (book_ambient_henkin_signature G) G v \<and>
...
proof -
  ...
  obtain u where maximal_u: "book_C_closed_maximal_extension \<Sigma> G (insert (book_not G P) ?K) u"
    and negative_u: "book_not G P \<in> u" and kernel_u: "?K \<subseteq> u"
    using book_C_closed_successor_exists[OF rich world pl closed missing] by blast
  have uc: "book_C_theory_consistent \<Sigma> G u" by (rule book_C_closed_maximal_consistent[OF rich maximal_u])
  have udata: "book_closed_formula_set \<Sigma> G u" by (rule book_C_closed_maximal_data(1)[OF maximal_u])
  have ul: "book_theory_formula \<Sigma> G A" if "A \<in> u" for A
    by (rule conjunct1[OF book_closed_formula_set_member[OF udata that]])
  obtain v where maximal_v: "book_C_closed_maximal_extension ?\<Omega> G {} v"
    and vc: "book_C_theory_consistent ?\<Omega> G v"
    and originals: "\<forall>A\<in>u. book_universal_closure G A \<in> v"
    and witnesses: "book_closed_constant_witness_complete ?\<Omega> G v"
    using book_C_ambient_henkin_extension_exists[OF rich ul uc] by blast
  ...
  have H_maximal: "book_closed_maximal_extension ?\<Omega> G (book_C_closed_theorems ?\<Omega> G \<union> {}) v"
    using maximal_v unfolding book_C_closed_maximal_extension_def .
