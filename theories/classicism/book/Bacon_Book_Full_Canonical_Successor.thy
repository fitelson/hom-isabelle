theory Bacon_Book_Full_Canonical_Successor
  imports Bacon_Book_Full_Canonical_World_Existence
begin
...
proof -
  ...
  obtain M where maximal: "book_full_C_closed_maximal_extension ?\<Omega> G {} M"
    ...
    using names.book_full_C_ambient_henkin_successor_exists[
      OF rich book_full_C_canonical_world_data(4)[OF world] pl closed missing] by blast
  have base: "\<Sigma> \<sigma> \<subseteq> ?\<Omega> \<sigma>" for \<sigma>
    by (rule subset_trans[OF book_full_C_canonical_world_data(1)[OF world] names.book_ambient_henkin_signature_contains])
  ...
  proof (unfold book_C_canonical_le_def, intro allI impI)
    ...
    have al: "book_theory_formula (fst w) G A" and ac: "named_fv A = {}"
      by (rule book_full_C_canonical_box_member_data[OF rich world boxed])+
