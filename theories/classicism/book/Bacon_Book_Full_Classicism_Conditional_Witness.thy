theory Bacon_Book_Full_Classicism_Conditional_Witness
  imports Bacon_Book_Full_Classicism_Fresh_Background
begin

section \<open>One fresh conditional witness preserves consistency over the enlarged full C\<close>

theorem book_full_C_consistent_conditional_witness:
  ...
  shows "book_full_C_theory_consistent (book_add_constant \<Sigma> c \<sigma>) G (insert (book_witness_axiom G \<sigma> F c) S)"
proof -
  let ?\<Omega> = "book_add_constant \<Sigma> c \<sigma>"
  let ?Cold = "{A. book_full_C_proves \<Sigma> G A}"
  let ?Cnew = "{A. book_full_C_proves ?\<Omega> G A}"
  let ?W = "book_witness_axiom G \<sigma> F c"
  have old_consistent: "book_theory_consistent \<Sigma> G (?Cold \<union> S)"
    using consistent by (simp add: book_full_C_theory_consistent_def book_full_C_theory_derivable_def book_theory_consistent_def)
  have old_language: "book_theory_formula \<Sigma> G A" if "A \<in> ?Cold \<union> S" for A
    using that language book_full_C_proves_language[OF rich] by blast
  ...
  proof (unfold book_full_C_theory_consistent_def, rule notI)
    assume contradiction: "book_full_C_theory_derivable ?\<Omega> G (insert ?W S) (book_bottom G)"
    have proof_new: "book_theory_derivable ?\<Omega> G (?Cnew \<union> insert ?W S) (book_bottom G)"
      using contradiction unfolding book_full_C_theory_derivable_def .
    ...
    proof (rule book_theory_derivable_cut[OF proof_new])
      ...
      proof (cases "book_full_C_proves ?\<Omega> G A")
        case True
        show ?thesis by (rule book_full_C_new_constant_theorem_from_old_background[OF rich True])
      next
        case False
