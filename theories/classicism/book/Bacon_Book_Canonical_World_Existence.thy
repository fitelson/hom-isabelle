theory Bacon_Book_Canonical_World_Existence
  imports Bacon_Book_Canonical_Worlds Bacon_Book_Countable_Signature_Recoding
begin

context book_countable_ambient_signature
begin

theorem book_C_canonical_world_exists:
  ...
  shows "\<exists>w\<in>book_C_canonical_worlds \<Sigma> B G. \<forall>A\<in>S. book_universal_closure G A \<in> snd w"
proof -
  ...
  obtain M where maximal: "book_C_closed_maximal_extension ?\<Omega> G {} M"
    ...
    using book_C_ambient_henkin_extension_exists[OF rich language consistent] by blast
  have world: "(?\<Omega>, M) \<in> book_C_canonical_worlds \<Sigma> B G"
...
theorem book_C_countable_canonical_world_exists:
  assumes rich: "sg_rich G" and small: "\<And>\<sigma>. countable (\<Sigma> \<sigma>)"
    ...
    and consistent: "book_C_theory_consistent \<Sigma> G S"
  shows "\<exists>w\<in>book_C_canonical_worlds (book_countable_signature_image \<Sigma>) (\<lambda>_. UNIV) G.
...
proof -
  ...
  have pc: "book_C_theory_consistent ?\<Omega> G ?S"
    using consistent book_C_countable_signature_consistency_iff[OF rich small language] by blast
  ...
  qed
  show ?thesis using names.book_C_canonical_world_exists[OF rich pl pc] by blast
qed
