theory Bacon_Book_Full_Canonical_Frame
  imports Bacon_Book_Full_Canonical_Successor
begin

section \<open>The root's future and Proposition 18.3\<close>

definition book_full_C_rooted_worlds where
  "book_full_C_rooted_worlds \<Sigma> B G actual =
...
abbreviation worlds where "worlds \<equiv> book_full_C_rooted_worlds \<Sigma> B G actual"
abbreviation le where "le \<equiv> book_C_canonical_le G"

lemma book_full_C_rooted_world_data:
  "w \<in> worlds \<Longrightarrow> w \<in> book_full_C_canonical_worlds \<Sigma> B G"
  "w \<in> worlds \<Longrightarrow> le actual w"
  unfolding book_full_C_rooted_worlds_def by blast+

theorem book_full_C_root_is_world:
  "actual \<in> worlds"
  using root_world book_full_C_canonical_le_refl[OF rich root_world] unfolding book_full_C_rooted_worlds_def by blast

theorem book_full_C_rooted_refl:
  "w \<in> worlds \<Longrightarrow> le w w"
  by (rule book_full_C_canonical_le_refl[OF rich book_full_C_rooted_world_data(1)]; assumption)

theorem book_full_C_rooted_trans:
  "w \<in> worlds \<Longrightarrow> le w v \<Longrightarrow> le v u \<Longrightarrow> le w u"
  by (rule book_full_C_canonical_le_trans[OF rich book_full_C_rooted_world_data(1)]; assumption)

theorem book_full_C_rooted_future_closed:
  assumes source: "w \<in> worlds" and target: "v \<in> book_full_C_canonical_worlds \<Sigma> B G"
    and access: "le w v"
  shows "v \<in> worlds"
proof -
  have from_root: "le actual v"
    by (rule book_full_C_canonical_le_trans[OF rich root_world book_full_C_rooted_world_data(2)[OF source] access])
  show ?thesis using target from_root unfolding book_full_C_rooted_worlds_def by blast
qed

theorem book_full_C_rooted_successor_exists:
...
proof -
  obtain v where vw: "v \<in> book_full_C_canonical_worlds \<Sigma> B G" and access: "le w v"
    and negative: "book_not G P \<in> snd v" and absent: "P \<notin> snd v"
    using book_full_C_canonical_successor_exists[OF rich book_full_C_rooted_world_data(1)[OF world] pl closed missing] by blast
  have inside: "v \<in> worlds" by (rule book_full_C_rooted_future_closed[OF world vw access])
...
next
  ...
  proof (rule ccontr)
    ...
    obtain v where vw: "v \<in> worlds" and access: "le w v" and absent: "P \<notin> snd v"
      using book_full_C_rooted_successor_exists[OF world pl closed missing] by blast
...
theorem book_full_C_canonical_frame_exists:
  ...
  shows "\<exists>actual. book_full_C_canonical_frame \<Sigma> B G actual \<and>
    (\<forall>A\<in>S. book_universal_closure G A \<in> snd actual)"
proof -
  obtain actual where aw: "actual \<in> book_full_C_canonical_worlds \<Sigma> B G"
  ...
  have frame: "book_full_C_canonical_frame \<Sigma> B G actual" by (unfold_locales; rule rich aw)
