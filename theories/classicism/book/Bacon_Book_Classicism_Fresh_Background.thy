theory Bacon_Book_Classicism_Fresh_Background
  imports Bacon_Book_Classicism_Theory_Retraction
    Bacon_Book_Environment_Development.Bacon_Book_Conditional_Witness
begin

section \<open>Recovering the new C background from old C theorems\<close>

lemma book_C_restore_one_constant:
...
theorem book_C_new_constant_theorem_from_old_background:
  assumes rich: "sg_rich G"
    and derivation: "book_C_proves (book_add_constant \<Sigma> c \<sigma>) G A"
  shows "book_theory_derivable (book_add_constant \<Sigma> c \<sigma>) G
    ({B. book_C_proves \<Sigma> G B} \<union> S) A"
proof -
  let ?\<Omega> = "book_add_constant \<Sigma> c \<sigma>"
  obtain N where support: "book_C_retraction_support \<Sigma> G A N"
    using book_C_retraction_support_exists[where \<Omega>=\<Sigma>, OF rich derivation] by blast
  have finite: "finite N" and contains: "named_vars A \<subseteq> N"
    using support unfolding book_C_retraction_support_def by blast+
  ...
  have old_C: "book_C_proves \<Sigma> G (named_retract \<Sigma> ?v A)"
    by (rule book_C_retraction_apply[OF support stock avoid])
  have old_type: "book_theory_formula \<Sigma> G (named_retract \<Sigma> ?v A)"
    by (rule book_C_proves_language[OF rich old_C])
  ...
  have old_member: "named_retract \<Sigma> ?v A \<in> {B. book_C_proves \<Sigma> G B} \<union> S"
    by (rule UnI1; simp only: mem_Collect_eq; rule old_C)
  have premise: "book_theory_derivable ?\<Omega> G ({B. book_C_proves \<Sigma> G B} \<union> S) (named_retract \<Sigma> ?v A)"
  ...
  have restored: "book_theory_derivable ?\<Omega> G ({B. book_C_proves \<Sigma> G B} \<union> S)
  ...
  have original_names: "named_in_signature ?\<Omega> A"
    by (rule book_language_signature[OF book_C_proves_language[OF rich derivation]])
  have fresh: "?v \<sigma> \<notin> named_vars A" using avoid contains by blast
  show ?thesis using restored by (simp only: book_C_restore_one_constant[
