theory Bacon_Source_Relational_Subcategory
  imports Bacon_Source_Relational_Category
begin
...
text \<open>
  ...
  Objects are independently R-valid model records on the specified common carrier;
  ...
  The F validator and F homomorphism predicate are not used. Type actions
  outside R have empty fibers on objects; only R types belong to the
  source language. No profile-separation or R model-extension result is claimed.
\<close>

definition paper_R_bbk_subcategory ::
  ...
  "paper_R_bbk_subcategory \<Sigma> G Obj Arrows \<longleftrightarrow>
    (\<forall>M\<in>Obj. paper_R_bbk_data_valid \<Sigma> G M) \<and>
    Arrows \<subseteq> paper_R_bbk_arrows \<Sigma> G Obj \<and>
...
lemma paper_R_bbk_subcategory_models:
  assumes subcategory: "paper_R_bbk_subcategory \<Sigma> G Obj Arrows" and member: "M \<in> Obj"
  shows "paper_R_bbk_data_valid \<Sigma> G M"
  using subcategory member unfolding paper_R_bbk_subcategory_def by blast

lemma paper_R_bbk_subcategory_arrow:
  assumes subcategory: "paper_R_bbk_subcategory \<Sigma> G Obj Arrows" and arrow: "f \<in> Arrows"
  shows "f \<in> paper_R_bbk_arrows \<Sigma> G Obj"
  using subcategory arrow unfolding paper_R_bbk_subcategory_def by blast

theorem paper_R_bbk_full_subcategory:
  assumes models: "\<And>M. M \<in> Obj \<Longrightarrow> paper_R_bbk_data_valid \<Sigma> G M"
  shows "paper_R_bbk_subcategory \<Sigma> G Obj (paper_R_bbk_arrows \<Sigma> G Obj)"
...
next
  show "paper_R_bbk_arrows \<Sigma> G Obj \<subseteq> paper_R_bbk_arrows \<Sigma> G Obj" by (rule subset_refl)
next
  show "\<forall>M\<in>Obj. paper_typed_identity paper_bbk_domain M \<in> paper_R_bbk_arrows \<Sigma> G Obj"
    by (intro ballI, rule paper_R_bbk_identity_arrow, assumption, rule models, assumption)
next
  show "\<forall>f\<in>paper_R_bbk_arrows \<Sigma> G Obj. \<forall>g\<in>paper_R_bbk_arrows \<Sigma> G Obj.
    paper_arrow_target f = paper_arrow_source g \<longrightarrow>
    paper_typed_compose paper_bbk_domain g f \<in> paper_R_bbk_arrows \<Sigma> G Obj"
    by (intro ballI impI, rule paper_R_bbk_compose_arrow; assumption)
qed

theorem paper_R_bbk_subcategory_category:
  assumes subcategory: "paper_R_bbk_subcategory \<Sigma> G Obj Arrows"
...
proof -
  have models: "paper_R_bbk_data_valid \<Sigma> G M" if "M \<in> Obj" for M
    by (rule paper_R_bbk_subcategory_models[OF subcategory that])
  interpret Full: paper_category Obj "paper_R_bbk_arrows \<Sigma> G Obj" paper_arrow_source paper_arrow_target
    "paper_typed_compose paper_bbk_domain" "paper_typed_identity paper_bbk_domain"
    by (rule paper_R_bbk_record_category[OF models])
  have subset: "Arrows \<subseteq> paper_R_bbk_arrows \<Sigma> G Obj"
    using subcategory unfolding paper_R_bbk_subcategory_def by blast
  have ids: "paper_typed_identity paper_bbk_domain M \<in> Arrows" if "M \<in> Obj" for M
    using subcategory that unfolding paper_R_bbk_subcategory_def by blast
...
theorem paper_R_bbk_selected_type_action:
  assumes subcategory: "paper_R_bbk_subcategory \<Sigma> G Obj Arrows"
...
proof -
  interpret Category: paper_category Obj Arrows paper_arrow_source paper_arrow_target
    "paper_typed_compose paper_bbk_domain" "paper_typed_identity paper_bbk_domain"
    by (rule paper_R_bbk_subcategory_category[OF subcategory])
  show ?thesis
  proof unfold_locales
    ...
    have typed_arrow: "h \<in> paper_typed_arrows Obj paper_bbk_domain"
      by (rule paper_R_bbk_arrows_typed[OF paper_R_bbk_subcategory_arrow[OF subcategory arrow]])
