theory Bacon_Source_Relational_Category
  imports Bacon_Source_Relational_Arrows Bacon_Source_Typed_Map_Category Bacon_Source_Subcategory
    Bacon_Source_Action
begin
...
text \<open>
  Let Obj be a set of independently R-valid BBK model records for the common
  ...
  No F validator, quasi-Fregean, quasi-functional or intensional assumption occurs.
  This is the full-hom category on the supplied records, not a category of
  all models or a completed R profile/action-model representation.
\<close>

theorem paper_R_bbk_record_category:
  assumes models: "\<And>M. M \<in> Obj \<Longrightarrow> paper_R_bbk_data_valid \<Sigma> G M"
  shows "paper_category Obj (paper_R_bbk_arrows \<Sigma> G Obj) paper_arrow_source paper_arrow_target
...
proof -
  ...
  show ?thesis
  proof (rule Typed.paper_category_restrict_arrows[OF paper_R_bbk_arrows_subset])
    fix M
    assume member: "M \<in> Obj"
    show "paper_typed_identity paper_bbk_domain M \<in> paper_R_bbk_arrows \<Sigma> G Obj"
      by (rule paper_R_bbk_identity_arrow[OF member models[OF member]])
  next
    fix f g
    assume first: "f \<in> paper_R_bbk_arrows \<Sigma> G Obj"
      and second: "g \<in> paper_R_bbk_arrows \<Sigma> G Obj"
      and meeting: "paper_arrow_target f = paper_arrow_source g"
    show "paper_typed_compose paper_bbk_domain g f \<in> paper_R_bbk_arrows \<Sigma> G Obj"
      by (rule paper_R_bbk_compose_arrow[OF first second meeting])
  qed
qed
...
text \<open>
  ...
  The theorem covers each R type. Its unrestricted type parameter also
  gives the empty action at non-R types, solely because the represented
  domains there are empty; it adds no non-R source types.
\<close>

theorem paper_R_bbk_type_action:
  assumes models: "\<And>M. M \<in> Obj \<Longrightarrow> paper_R_bbk_data_valid \<Sigma> G M"
  shows "paper_action Obj (paper_R_bbk_arrows \<Sigma> G Obj) paper_arrow_source paper_arrow_target
    (paper_typed_compose paper_bbk_domain) (paper_typed_identity paper_bbk_domain)
    (\<lambda>M. paper_bbk_domain M \<sigma>) (\<lambda>h. paper_arrow_map h \<sigma>)"
proof -
  interpret Category: paper_category Obj "paper_R_bbk_arrows \<Sigma> G Obj"
    paper_arrow_source paper_arrow_target "paper_typed_compose paper_bbk_domain"
    "paper_typed_identity paper_bbk_domain"
  ...
  proof unfold_locales
    fix h a
    assume arrow: "h \<in> paper_R_bbk_arrows \<Sigma> G Obj"
    ...
    show "paper_arrow_map h \<sigma> a \<in> paper_bbk_domain (paper_arrow_target h) \<sigma>"
      by (rule paper_typed_arrows_map[OF paper_R_bbk_arrows_typed[OF arrow] member])
  next
    fix M a
