theory Bacon_Source_Relational_Application_Profile_Action
  imports Bacon_Source_Relational_Application_Profile Bacon_Source_Exponential_Action
begin
...
text \<open>
  ...
  functions to the independently validated selected R BBK category;
  no F model or fullness is inferred.
\<close>

theorem paper_R_app_profile_on_exponential:
  ...
  shows "paper_R_app_profile_on \<Sigma> G Arrows M \<sigma> \<tau> d \<in>
...
proof (rule paper_exponential_fiberI)
  ...
  show "paper_R_app_profile_on \<Sigma> G Arrows M \<sigma> \<tau> d p = undefined"
  proof (cases p)
    case (Pair h a)
    ...
    show ?thesis by (simp only: Pair; rule paper_R_app_profile_on_outside[OF absent])
  qed
next
  fix h a
  assume arrow: "h \<in> Arrows" and source: "paper_arrow_source h = M"
    and argument: "a \<in> paper_bbk_domain (paper_arrow_target h) \<sigma>"
  show "paper_R_app_profile_on \<Sigma> G Arrows M \<sigma> \<tau> d (h,a) \<in>
    paper_bbk_domain (paper_arrow_target h) \<tau>"
    by (rule paper_R_app_profile_on_type[OF category rt member arrow source argument])
next
  fix h k a
  assume first: "h \<in> Arrows" and source: "paper_arrow_source h = M"
    ...
    and argument: "a \<in> paper_bbk_domain (paper_arrow_target h) \<sigma>"
  show "paper_arrow_map k \<tau> (paper_R_app_profile_on \<Sigma> G Arrows M \<sigma> \<tau> d (h,a)) =
    paper_R_app_profile_on \<Sigma> G Arrows M \<sigma> \<tau> d
      (paper_typed_compose paper_bbk_domain k h, paper_arrow_map k \<sigma> a)"
    by (rule paper_R_app_profile_on_coherent[OF category rt member first source argument second meeting])
qed
...
theorem paper_R_app_profile_on_naturality:
  ...
  shows "paper_R_app_profile_on \<Sigma> G Arrows (paper_arrow_target f) \<sigma> \<tau>
    ...
    paper_exponential_transport Arrows paper_arrow_source paper_arrow_target
      ...
      (paper_R_app_profile_on \<Sigma> G Arrows (paper_arrow_source f) \<sigma> \<tau> d)"
proof -
  ...
  show ?thesis
  proof (rule ext)
    fix p :: "('c,'v) paper_R_bbk_arrow \<times> 'v"
    obtain h a where pair: "p = (h,a)" by (cases p) auto
    show "paper_R_app_profile_on \<Sigma> G Arrows (paper_arrow_target f) \<sigma> \<tau>
        (paper_arrow_map f (Arr \<sigma> \<tau>) d) p =
      paper_exponential_transport Arrows paper_arrow_source paper_arrow_target
        (paper_typed_compose paper_bbk_domain) (\<lambda>N. paper_bbk_domain N \<sigma>) f
        (paper_R_app_profile_on \<Sigma> G Arrows (paper_arrow_source f) \<sigma> \<tau> d) p"
    proof (cases "(h,a) \<in> paper_exponential_pairs Arrows paper_arrow_source paper_arrow_target
      ...
      show ?thesis
        by (simp only: pair paper_exponential_transport_on[OF True]
          paper_R_app_profile_on_value[OF ha source argument]
    ...
    next
      ...
      show ?thesis by (simp only: pair paper_exponential_transport_off[OF False]
        paper_R_app_profile_on_outside[OF absent])
    qed
  qed
qed
...
theorem paper_R_profile_exponential_action:
  assumes category: "paper_R_bbk_subcategory \<Sigma> G Obj Arrows"
    and rt: "paper_R_type (Arr \<sigma> \<tau>)"
  ...
  by (rule paper_exponential_action_from_actions[
    OF paper_R_bbk_selected_type_action[where \<sigma>=\<sigma>, OF category]
      paper_R_bbk_selected_type_action[where \<sigma>=\<tau>, OF category]])

section \<open>Quasi-functionality remains a separate condition\<close>

text \<open>
  An R category is quasi-functional when app𝒞M is injective at every
  σ→τ∈R (Definition 3.11, p.50). Because profiles have
  ...
  The quantification here is restricted to R arrow types, and the profile
  uses application reconstructed from the independent R interpretation.
  No F validator or F profile theorem is used. The predicate alone does
  not certify a category; the selected-category hypothesis is separate.
\<close>

definition paper_R_quasi_functional_on ::
  ...
  "paper_R_quasi_functional_on \<Sigma> G Obj Arrows \<longleftrightarrow>
    (\<forall>M\<in>Obj. \<forall>\<sigma> \<tau>. paper_R_type (Arr \<sigma> \<tau>) \<longrightarrow> inj_on (paper_R_app_profile_on \<Sigma> G Arrows M \<sigma> \<tau>)
      (paper_bbk_domain M (Arr \<sigma> \<tau>)))"

lemma paper_R_quasi_functional_on_separates:
  assumes quasi: "paper_R_quasi_functional_on \<Sigma> G Obj Arrows"
    and object: "M \<in> Obj"
    and rt: "paper_R_type (Arr \<sigma> \<tau>)"
  ...
  shows "d = e"
proof -
  have injective: "inj_on (paper_R_app_profile_on \<Sigma> G Arrows M \<sigma> \<tau>)
    (paper_bbk_domain M (Arr \<sigma> \<tau>))"
    using quasi object rt unfolding paper_R_quasi_functional_on_def by blast
