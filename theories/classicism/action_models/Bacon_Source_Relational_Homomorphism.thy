theory Bacon_Source_Relational_Homomorphism
...
text \<open>
  ...
  Scope: the first-class paper basis over R, arbitrary nonlogical signature
  Σ, and the same named variable stock G on both sides. Domain maps are
  tested on actual members of the represented type family. For R models,
  all non-R domains are empty, so this imposes no off-R semantic condition.
  The raw predicate does not itself certify that normalization or any model
  axiom. No F model, F language, richness, injectivity, surjectivity, truth
  preservation, or Functionality is assumed. Status: independent R maps,
  not an R↔F bridge or a category construction.
\<close>

definition paper_R_bbk_homomorphism ::
  ...
  "paper_R_bbk_homomorphism \<Sigma> G D J E K h \<longleftrightarrow>
    (\<forall>\<sigma> a. a \<in> D \<sigma> \<longrightarrow> h \<sigma> a \<in> E \<sigma>) \<and>
    (\<forall>\<sigma> A g. paper_R_in_language \<Sigma> G A \<sigma> \<longrightarrow>
      named_env_typed D G g \<longrightarrow> named_adequate g A \<longrightarrow>
      h \<sigma> (J g A) = K (paper_hom_assignment G h g) A)"

lemma paper_R_bbk_homomorphismI:
  assumes domains: "\<And>\<sigma> a. a \<in> D \<sigma> \<Longrightarrow> h \<sigma> a \<in> E \<sigma>"
    and terms: "\<And>\<sigma> A g. paper_R_in_language \<Sigma> G A \<sigma> \<Longrightarrow>
      named_env_typed D G g \<Longrightarrow> named_adequate g A \<Longrightarrow>
      h \<sigma> (J g A) = K (paper_hom_assignment G h g) A"
  shows "paper_R_bbk_homomorphism \<Sigma> G D J E K h"
  unfolding paper_R_bbk_homomorphism_def
  by (intro conjI allI impI; (rule domains | rule terms); assumption)

lemma paper_R_bbk_homomorphism_domain:
  assumes hom: "paper_R_bbk_homomorphism \<Sigma> G D J E K h"
    and member: "a \<in> D \<sigma>"
  shows "h \<sigma> a \<in> E \<sigma>"
  using hom member unfolding paper_R_bbk_homomorphism_def by blast

lemma paper_R_bbk_homomorphismI_R_domains:
  assumes outside: "\<And>\<sigma>. \<not> paper_R_type \<sigma> \<Longrightarrow> D \<sigma> = {}"
    and maps: "\<And>\<sigma> a. paper_R_type \<sigma> \<Longrightarrow> a \<in> D \<sigma> \<Longrightarrow> h \<sigma> a \<in> E \<sigma>"
    and terms: "\<And>\<sigma> A g. paper_R_in_language \<Sigma> G A \<sigma> \<Longrightarrow>
      named_env_typed D G g \<Longrightarrow> named_adequate g A \<Longrightarrow>
      h \<sigma> (J g A) = K (paper_hom_assignment G h g) A"
  shows "paper_R_bbk_homomorphism \<Sigma> G D J E K h"
proof (rule paper_R_bbk_homomorphismI)
  ...
  show "h \<sigma> a \<in> E \<sigma>"
  proof (cases "paper_R_type \<sigma>")
    case True
    show ?thesis by (rule maps[OF True member])
  next
    case False
    have "D \<sigma> = {}" by (rule outside[OF False])
    with member show ?thesis by simp
  qed
next
  fix \<sigma> A g
  assume language: "paper_R_in_language \<Sigma> G A \<sigma>"
    and typed: "named_env_typed D G g" and adequate: "named_adequate g A"
  show "h \<sigma> (J g A) = K (paper_hom_assignment G h g) A"
    by (rule terms[OF language typed adequate])
qed

lemma paper_R_bbk_homomorphism_denote:
  assumes hom: "paper_R_bbk_homomorphism \<Sigma> G D J E K h"
    and language: "paper_R_in_language \<Sigma> G A \<sigma>"
    and typed: "named_env_typed D G g" and adequate: "named_adequate g A"
  shows "h \<sigma> (J g A) = K (paper_hom_assignment G h g) A"
  using hom language typed adequate unfolding paper_R_bbk_homomorphism_def by blast

lemma paper_R_bbk_homomorphism_assignment_typed:
  assumes hom: "paper_R_bbk_homomorphism \<Sigma> G D J E K h"
...
proof (rule paper_hom_assignment_typed[where D=D and E=E, OF _ typed])
  fix \<sigma> a
  assume member: "a \<in> D \<sigma>"
  show "h \<sigma> a \<in> E \<sigma>" by (rule paper_R_bbk_homomorphism_domain[OF hom member])
qed

lemma paper_R_bbk_homomorphism_assignment_adequate:
  assumes adequate: "named_adequate g A"
  shows "named_adequate (paper_hom_assignment G h g) A"
...
theorem paper_R_bbk_homomorphism_identity:
  "paper_R_bbk_homomorphism \<Sigma> G D J D J (\<lambda>\<sigma> a. a)"
proof (rule paper_R_bbk_homomorphismI)
  fix \<sigma> a
  assume member: "a \<in> D \<sigma>"
...
next
  fix \<sigma> A g
  assume "paper_R_in_language \<Sigma> G A \<sigma>"
...
theorem paper_R_bbk_homomorphism_compose:
  assumes first: "paper_R_bbk_homomorphism \<Sigma> G D J E K h"
    and second: "paper_R_bbk_homomorphism \<Sigma> G E K F L k"
  shows "paper_R_bbk_homomorphism \<Sigma> G D J F L (\<lambda>\<sigma> a. k \<sigma> (h \<sigma> a))"
proof (rule paper_R_bbk_homomorphismI)
  fix \<sigma> a
  assume member: "a \<in> D \<sigma>"
  have intermediate: "h \<sigma> a \<in> E \<sigma>"
    by (rule paper_R_bbk_homomorphism_domain[OF first member])
  show "k \<sigma> (h \<sigma> a) \<in> F \<sigma>"
    by (rule paper_R_bbk_homomorphism_domain[OF second intermediate])
next
  fix \<sigma> A g
  assume language: "paper_R_in_language \<Sigma> G A \<sigma>"
    and typed: "named_env_typed D G g" and adequate: "named_adequate g A"
  have intermediate_typed: "named_env_typed E G (paper_hom_assignment G h g)"
    by (rule paper_R_bbk_homomorphism_assignment_typed[OF first typed])
  ...
  have first_equation: "h \<sigma> (J g A) = K (paper_hom_assignment G h g) A"
    by (rule paper_R_bbk_homomorphism_denote[OF first language typed adequate])
  have second_equation: "k \<sigma> (K (paper_hom_assignment G h g) A) =
    L (paper_hom_assignment G k (paper_hom_assignment G h g)) A"
    by (rule paper_R_bbk_homomorphism_denote[OF second language intermediate_typed intermediate_adequate])
...
theorem paper_R_bbk_homomorphism_cong:
  assumes hom: "paper_R_bbk_homomorphism \<Sigma> G D J E K h"
    and source_typed: "\<And>A \<sigma> g. paper_R_in_language \<Sigma> G A \<sigma> \<Longrightarrow>
  ...
  shows "paper_R_bbk_homomorphism \<Sigma> G D J E K k"
proof (rule paper_R_bbk_homomorphismI)
  fix \<sigma> a
  assume member: "a \<in> D \<sigma>"
  have image: "h \<sigma> a \<in> E \<sigma>" by (rule paper_R_bbk_homomorphism_domain[OF hom member])
