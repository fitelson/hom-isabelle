theory Bacon_Source_Relational_Normalization_Validity
  imports Bacon_Source_Relational_Normalization_Inputs
begin
...
text \<open>
  ...
  every field of the independent R named BBK interface, including
  nonemptiness only for σ∈R and emptiness off R.
  Source: Bacon–Dorr Definition 3.1, pp.43–44, with the type
  system of §1.1, p.5. Off-R emptiness is retained as the explicit
  representation convention, not an added source condition.
  ...
  is introduced. Neither an F model nor a total assignment completion
  is used; assigned variables acquire their R guard from domain_empty.
\<close>

theorem paper_R_bbk_normalize_valid:
  assumes valid: "paper_R_bbk_data_valid \<Sigma> G M"
  shows "paper_R_bbk_data_valid \<Sigma> G (paper_R_bbk_normalize \<Sigma> G M)"
proof -
  let ?D = "paper_bbk_domain M"
  let ?J = "paper_bbk_denote (paper_R_bbk_normalize \<Sigma> G M)"
  let ?V = "paper_bbk_valuation (paper_R_bbk_normalize \<Sigma> G M)"
  let ?OldJ = "paper_bbk_denote M"
  let ?OldV = "paper_bbk_valuation M"
  interpret Source: paper_R_bbk_model \<Sigma> G ?D ?OldJ ?OldV
    by (rule paper_R_bbk_data_model[OF valid])
  have normalized_model: "paper_R_bbk_model \<Sigma> G ?D ?J ?V"
  proof (rule paper_R_bbk_model.intro)
    show "paper_R_rich G" by (rule Source.stock_rich)
  next
    fix \<sigma>
    assume rt: "paper_R_type \<sigma>"
    show "?D \<sigma> \<noteq> {}" by (rule Source.domain_nonempty[OF rt])
  next
    fix \<sigma>
    assume outside: "\<not> paper_R_type \<sigma>"
    show "?D \<sigma> = {}" by (rule Source.domain_empty[OF outside])
  next
    fix A \<sigma> g
    assume al: "paper_R_in_language \<Sigma> G A \<sigma>"
      and gt: "named_env_typed ?D G g" and ag: "named_adequate g A"
    have original: "?OldJ g A \<in> ?D \<sigma>"
      by (rule paper_R_bbk_data_denote_type[OF valid al gt ag])
    show "?J g A \<in> ?D \<sigma>"
      using original by (simp only: paper_R_bbk_normalize_denote[OF al gt ag])
  next
    fix g n a
    assume gt: "named_env_typed ?D G g" and assigned: "g n = Some a"
    have variable_R: "paper_R_type (G n)"
      by (rule Source.paper_R_assigned_variable_type[OF gt assigned])
    have vl: "paper_R_in_language \<Sigma> G (NVar n) (G n)"
      by (rule paper_R_normalization_Var[where G=G and n=n, OF variable_R])
    ...
    show "?J g (NVar n) = a"
      using original by (simp only: paper_R_bbk_normalize_denote[OF vl gt adequate])
  next
    fix F A H B \<sigma> \<tau> \<upsilon> \<rho> g h
    assume fl: "paper_R_in_language \<Sigma> G F (Arr \<sigma> \<tau>)"
    ...
    have first_adequate: "named_adequate g F \<and> named_adequate g A"
      using fg by (simp only: paper_R_bbk_adequate_App_iff)
    ...
    have second_adequate: "named_adequate h H \<and> named_adequate h B"
      using hh by (simp only: paper_R_bbk_adequate_App_iff)
    ...
    have old_heads: "?OldJ g F = ?OldJ h H"
      using heads by (simp only: paper_R_bbk_normalize_denote[OF fl gt fa]
        paper_R_bbk_normalize_denote[OF hl ht ha])
    have old_arguments: "?OldJ g A = ?OldJ h B"
      using arguments by (simp only: paper_R_bbk_normalize_denote[OF al gt aa]
        paper_R_bbk_normalize_denote[OF bl ht ba])
    ...
    show "?J g (NApp F A) = ?J h (NApp H B)"
      using original by (simp only: paper_R_bbk_normalize_denote[OF paper_R_normalization_App[OF fl al] gt fg]
        paper_R_bbk_normalize_denote[OF paper_R_normalization_App[OF hl bl] ht hh])
  next
    fix A \<sigma> g h
    assume al: "paper_R_in_language \<Sigma> G A \<sigma>"
      and gt: "named_env_typed ?D G g" and ht: "named_env_typed ?D G h"
    ...
    show "?J g A = ?J h A"
      using original by (simp only: paper_R_bbk_normalize_denote[OF al gt ag] paper_R_bbk_normalize_denote[OF al ht ah])
  next
    fix A B \<sigma> g
    ...
    show "?J g A = ?J g B"
      using original by (simp only: paper_R_bbk_normalize_denote[OF al gt ag] paper_R_bbk_normalize_denote[OF bl gt bg])
  next
    fix A g
    assume al: "paper_R_in_language \<Sigma> G A Prop"
      and gt: "named_env_typed ?D G g" and ag: "named_adequate g A"
    ...
    have lhs: "?V (?J g (NApp (NLogical SNot) A)) = ?OldV (?OldJ g (NApp (NLogical SNot) A))"
      by (rule paper_R_bbk_normalize_unary_truth[where l=SNot, OF valid op_type al gt ag])
    ...
    show "?V (?J g (NApp (NLogical SNot) A)) = (\<not> ?V (?J g A))"
      using original by (simp only: lhs paper_R_bbk_normalize_truth[OF valid al gt ag])
  next
    fix A B g
    assume al: "paper_R_in_language \<Sigma> G A Prop"
    ...
    have lhs: "?V (?J g (NApp (NApp (NLogical SAnd) A) B)) = ?OldV (?OldJ g (NApp (NApp (NLogical SAnd) A) B))"
      by (rule paper_R_bbk_normalize_binary_truth[where l=SAnd, OF valid op_type al bl gt ag bg])
    ...
    show "?V (?J g (NApp (NApp (NLogical SAnd) A) B)) = (?V (?J g A) \<and> ?V (?J g B))"
      using original by (simp only: lhs paper_R_bbk_normalize_truth[OF valid al gt ag]
        paper_R_bbk_normalize_truth[OF valid bl gt bg])
  next
    fix A B g
    assume al: "paper_R_in_language \<Sigma> G A Prop"
      and bl: "paper_R_in_language \<Sigma> G B Prop"
    ...
    have lhs: "?V (?J g (NApp (NApp (NLogical SOr) A) B)) = ?OldV (?OldJ g (NApp (NApp (NLogical SOr) A) B))"
      by (rule paper_R_bbk_normalize_binary_truth[where l=SOr, OF valid op_type al bl gt ag bg])
  ...
  next
    ...
    have lhs: "?V (?J g (NApp (NLogical (SAll \<sigma>)) F)) = ?OldV (?OldJ g (NApp (NLogical (SAll \<sigma>)) F))"
      by (rule paper_R_bbk_normalize_unary_truth[where l="SAll \<sigma>", OF valid op_type fl gt fg])
    have rhs: "(\<forall>a\<in>?D \<sigma>. ?V (?J (g(n := Some a)) (NApp F (NVar n)))) =
      (\<forall>a\<in>?D \<sigma>. ?OldV (?OldJ (g(n := Some a)) (NApp F (NVar n))))"
      by (rule ball_cong[OF refl], rule paper_R_bbk_normalize_quantifier_instance[OF valid fl gt fg nt], assumption)
  ...
  next
    ...
    have lhs: "?V (?J g (NApp (NLogical (SEx \<sigma>)) F)) = ?OldV (?OldJ g (NApp (NLogical (SEx \<sigma>)) F))"
      by (rule paper_R_bbk_normalize_unary_truth[where l="SEx \<sigma>", OF valid op_type fl gt fg])
    have rhs: "(\<exists>a\<in>?D \<sigma>. ?V (?J (g(n := Some a)) (NApp F (NVar n)))) =
      (\<exists>a\<in>?D \<sigma>. ?OldV (?OldJ (g(n := Some a)) (NApp F (NVar n))))"
      by (rule bex_cong[OF refl], rule paper_R_bbk_normalize_quantifier_instance[OF valid fl gt fg nt], assumption)
  ...
  next
    ...
    have lhs: "?V (?J g (NApp (NApp (NLogical (SEq \<sigma>)) A) B)) =
      ...
      by (rule paper_R_bbk_normalize_binary_truth[where l="SEq \<sigma>", OF valid op_type al bl gt ag bg])
    ...
    show "?V (?J g (NApp (NApp (NLogical (SEq \<sigma>)) A) B)) = (?J g A = ?J g B)"
      using original by (simp only: lhs paper_R_bbk_normalize_denote[OF al gt ag]
        paper_R_bbk_normalize_denote[OF bl gt bg])
  qed
  show ?thesis using normalized_model
    by (simp only: paper_R_bbk_data_valid_def paper_R_bbk_normalize_domain)
qed
