theory Bacon_Book_Lambda_I_Closed_Maximal_Extension
  imports Bacon_Book_Lambda_I_Consistency_Unions "HOL.Zorn"
begin

section \<open>Maximal consistency among closed formulas\<close>

text \<open>
  A λI-consistent set S of typed closed λI formulas extends to a set M
  of typed closed λI formulas that is λI-consistent and maximal among
  λI-consistent sets of such formulas, in the SAME signature Σ and
  variable stock G. Consistency and maximality here are those of the λI
  calculus book_lambda_I_derivable, over λI formulas only; nonrelevant
  formulas are not members. Source role: the closed-formula completion stage
...
definition book_lambda_I_closed_formula_set ::
  "'c ssignature \<Rightarrow> sgcontext \<Rightarrow> 'c book_named_term set \<Rightarrow> bool" where
  "book_lambda_I_closed_formula_set \<Sigma> G S \<longleftrightarrow>
    (\<forall>A\<in>S. book_lambda_I_formula \<Sigma> G A \<and> named_fv A = {})"

lemma book_lambda_I_closed_formula_set_member:
  assumes closed_set: "book_lambda_I_closed_formula_set \<Sigma> G S" and member: "A \<in> S"
  shows "book_lambda_I_formula \<Sigma> G A \<and> named_fv A = {}"
  using closed_set member unfolding book_lambda_I_closed_formula_set_def by blast

lemma book_lambda_I_closed_formula_set_Union:
  assumes members: "\<And>U. U \<in> \<C> \<Longrightarrow> book_lambda_I_closed_formula_set \<Sigma> G U"
  shows "book_lambda_I_closed_formula_set \<Sigma> G (\<Union>\<C>)"
proof (unfold book_lambda_I_closed_formula_set_def, rule ballI)
  ...
  show "book_lambda_I_formula \<Sigma> G A \<and> named_fv A = {}"
    by (rule book_lambda_I_closed_formula_set_member[OF members[OF U_member] A_member])
qed

definition book_lambda_I_closed_maximal_extension ::
  ...
  "book_lambda_I_closed_maximal_extension \<Sigma> G S M \<longleftrightarrow>
    S \<subseteq> M \<and> book_lambda_I_closed_formula_set \<Sigma> G M \<and> book_lambda_I_consistent \<Sigma> G M \<and>
    (\<forall>U. M \<subseteq> U \<longrightarrow> book_lambda_I_closed_formula_set \<Sigma> G U \<longrightarrow>
...
theorem book_lambda_I_closed_maximal_extension_exists:
  assumes closed_S: "book_lambda_I_closed_formula_set \<Sigma> G S"
    and consistent_S: "book_lambda_I_consistent \<Sigma> G S"
  shows "\<exists>M. book_lambda_I_closed_maximal_extension \<Sigma> G S M"
proof -
  let ?F = "{U. S \<subseteq> U \<and> book_lambda_I_closed_formula_set \<Sigma> G U \<and> book_lambda_I_consistent \<Sigma> G U}"
  ...
  next
    ...
    have all_closed: "book_lambda_I_closed_formula_set \<Sigma> G U" if "U \<in> \<C>" for U
    proof -
      have member: "U \<in> ?F" by (rule subsetD[OF inside that])
      show ?thesis using member by simp
    qed
    have all_consistent: "book_lambda_I_consistent \<Sigma> G U" if "U \<in> \<C>" for U
    proof -
      have member: "U \<in> ?F" by (rule subsetD[OF inside that])
      show ?thesis using member by simp
    qed
    ...
    have closed_union: "book_lambda_I_closed_formula_set \<Sigma> G (\<Union>\<C>)"
      by (rule book_lambda_I_closed_formula_set_Union[OF all_closed])
    have consistent_union: "book_lambda_I_consistent \<Sigma> G (\<Union>\<C>)"
      by (rule book_lambda_I_consistent_chain_Union[OF False comparable all_consistent])
  ...
  have extends: "S \<subseteq> M" and closed_M: "book_lambda_I_closed_formula_set \<Sigma> G M"
    and consistent_M: "book_lambda_I_consistent \<Sigma> G M" using M_member by simp_all
  have maximal_closed: "\<forall>U. M \<subseteq> U \<longrightarrow> book_lambda_I_closed_formula_set \<Sigma> G U \<longrightarrow>
    book_lambda_I_consistent \<Sigma> G U \<longrightarrow> U = M"
  proof (intro allI impI)
    fix U
    assume contains_M: "M \<subseteq> U" and closed_U: "book_lambda_I_closed_formula_set \<Sigma> G U"
      and consistent_U: "book_lambda_I_consistent \<Sigma> G U"
  ...
  qed
  have result: "book_lambda_I_closed_maximal_extension \<Sigma> G S M"
    unfolding book_lambda_I_closed_maximal_extension_def
