theory Bacon_Book_Lambda_I_Finite_Witness_Family
  imports Bacon_Book_Lambda_I_Witness_Family_Syntax
begin

section \<open>Freshness in a partial witness signature\<close>

lemma book_lambda_I_witness_family_next_fresh:
...
text \<open>
  Let I be finite. Each closed λI predicate Fᵢ:τᵢ→t belongs to the OLD
  signature Σ; each cᵢ is fresh at τᵢ, and the names cᵢ are distinct on
  I. If S is a λI-consistent set of λI formulas of Σ, then S∪W[I] is
  λI-consistent in Σ[I]. Relevance of the predicates and premises and λI
  consistency are the extra hypotheses of this port.
...
theorem book_lambda_I_consistent_finite_witness_family:
  ...
  using finite_I predicates closed fresh injective lambda_I
...
next
  ...
  have tail_lambda_I: "book_lambda_I (F j)" if "j \<in> I" for j
    by (rule insert.prems(5), rule insertI2[OF that])
  have tail_injective: "inj_on c I"
    by (rule inj_on_subset[OF insert.prems(4) subset_insertI])
  have partial_consistent: "book_lambda_I_consistent ?\<Omega> G ?P"
    by (rule insert.IH[OF tail_predicates tail_closed tail_fresh tail_injective tail_lambda_I])
  have partial_language: "book_lambda_I_formula ?\<Omega> G A" if "A \<in> ?P" for A
    by (rule book_lambda_I_witness_family_premises_language[OF rich language tail_predicates tail_lambda_I that])
  ...
  have enlarged_predicate: "book_in_language book_minimal_logical_type UNIV ?\<Omega> G (F i) (Arr (\<tau> i) Prop)"
    by (rule book_lambda_I_language_signature_mono[OF old_predicate]; rule book_witness_family_signature_inclusion)
  have current_closed: "named_fv (F i) = {}" by (rule insert.prems(2)[OF i_member])
  have current_lambda_I: "book_lambda_I (F i)" by (rule insert.prems(5)[OF i_member])
  have current_fresh: "c i \<notin> ?\<Omega> (\<tau> i)"
    by (rule book_lambda_I_witness_family_next_fresh[
      where \<Sigma>=\<Sigma> and \<tau>=\<tau> and c=c and i=i and I=I,
      OF insert.prems(3)[OF i_member] insert.hyps(2) insert.prems(4)])
  have extended: "book_lambda_I_consistent (book_add_constant ?\<Omega> (c i) (\<tau> i)) G
    (insert (book_witness_axiom G (\<tau> i) (F i) (c i)) ?P)"
    by (rule book_lambda_I_consistent_conditional_witness[
      OF rich partial_consistent partial_language enlarged_predicate current_closed current_lambda_I current_fresh])
