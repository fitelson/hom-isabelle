theory Bacon_Book_Lambda_I_Conversion_Expanded_Model_Existence
  imports Bacon_Book_Lambda_I_Conversion_Model Bacon_Book_Lambda_I_Conversion_Truth_Projection Bacon_Book_Lambda_I_Closed_Henkin_Extension Bacon_Book_Lambda_I_Universal_Closure_Truth
begin
...
theorem book_lambda_I_henkin_conversion_original_valid:
  assumes rich: "sg_rich G"
    and language: "\<And>B. B \<in> S \<Longrightarrow> book_lambda_I_formula \<Sigma> G B"
    and maximal: "book_lambda_I_closed_maximal_extension (book_lambda_I_henkin_full_signature \<Sigma> G) G
    ...
    and member: "A \<in> S"
  shows "book_formula_valid (book_lambda_I_conversion_domain (book_lambda_I_henkin_full_signature \<Sigma> G) G) G
    (book_lambda_I_conversion_denote (book_lambda_I_henkin_full_signature \<Sigma> G) G)
    (book_lambda_I_conversion_valuation M) (book_constant_rename BookOriginal A)"
proof -
  ...
  interpret Model: book_lambda_I_model ?D "book_lambda_I_conversion_app ?\<Omega> G" ?\<Omega> G ?J
    "book_lambda_I_conversion_valuation M" "book_lambda_I_conversion_logical_value ?\<Omega> G"
    by (rule book_lambda_I_henkin_conversion_model[OF rich maximal witnesses])
  have original_image: "?A \<in> image (book_constant_rename BookOriginal) S"
    by (rule imageI[OF member])
  have in_full: "?A \<in> book_lambda_I_henkin_full_premises \<Sigma> G S"
    by (rule subsetD[OF book_lambda_I_henkin_original_in_full original_image])
  have al: "book_lambda_I_formula ?\<Omega> G ?A"
    by (rule book_lambda_I_henkin_full_premises_language[OF rich language in_full])
  have ul: "book_lambda_I_formula ?\<Omega> G (book_lambda_I_universal_closure G ?A)"
    by (rule book_lambda_I_universal_closure_language[OF al])
  have uc: "named_fv (book_lambda_I_universal_closure G ?A) = {}"
    by (rule book_lambda_I_universal_closure_closed)
  have um: "book_lambda_I_universal_closure G ?A \<in> M"
    by (rule book_lambda_I_closed_henkin_original[OF maximal member])
  have uv: "book_formula_valid ?D G ?J (book_lambda_I_conversion_valuation M) (book_lambda_I_universal_closure G ?A)"
    by (rule iffD2[OF book_lambda_I_henkin_conversion_closed_valid_iff[OF rich maximal ul uc] um])
  show ?thesis by (rule iffD1[OF Model.book_formula_valid_universal_closure_iff[OF al] uv])
qed

theorem book_lambda_I_henkin_expanded_model_exists:
  assumes rich: "sg_rich G"
...
proof -
  obtain M where maximal: "book_lambda_I_closed_maximal_extension (book_lambda_I_henkin_full_signature \<Sigma> G) G
      (book_lambda_I_henkin_closed_premises \<Sigma> G S) M"
    and witnesses: "book_lambda_I_closed_constant_witness_complete (book_lambda_I_henkin_full_signature \<Sigma> G) G M"
    using book_lambda_I_closed_henkin_extension_exists[OF rich language consistent] by blast
  show ?thesis
    by (rule exI[where x=M], rule conjI[OF book_lambda_I_henkin_conversion_model[OF rich maximal witnesses]],
        rule ballI, rule book_lambda_I_henkin_conversion_original_valid[OF rich language maximal witnesses], assumption)
qed
