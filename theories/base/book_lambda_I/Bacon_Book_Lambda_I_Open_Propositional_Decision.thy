theory Bacon_Book_Lambda_I_Open_Propositional_Decision
  imports Bacon_Book_Lambda_I_Theory_Variable_Substitution Bacon_Book_Lambda_I_Negation_Conversion
begin
...
lemma book_lambda_I_not_variable_subst:
...
lemma book_lambda_I_not_variable_free_for_iff:
...
text \<open>
  ...
  Source rules: Definition 5.1 and Definition 5.4, pp.98 and 102. The
  λI rules used here are those of Definition 9.8, p.197, restricted to
  λI formulas, i.e. the same schemas as Definition 5.1 (see
...
theorem book_lambda_I_open_positive_bottom:
  assumes rich: "sg_rich G"
    and positive: "book_lambda_I_derivable \<Sigma> G S (NVar (book_prop_name G))"
  shows "book_lambda_I_derivable \<Sigma> G S (book_bottom G)"
proof -
  ...
  have bottom: "book_in_language book_minimal_logical_type UNIV \<Sigma> G (book_bottom G) (G ?p)"
    by (simp only: ptype; rule conjunct1[OF book_lambda_I_bottom_language[OF rich]])
  have free_for: "named_free_for (book_bottom G) ?p (NVar ?p)" by simp
  have substituted: "book_lambda_I_derivable \<Sigma> G S (named_subst ?p (book_bottom G) (NVar ?p))"
    by (rule book_lambda_I_variable_substitution[OF rich positive bottom book_lambda_I_bottom free_for])
  show ?thesis using substituted by simp
qed

theorem book_lambda_I_open_negative_bottom:
  assumes rich: "sg_rich G"
    and negative: "book_lambda_I_derivable \<Sigma> G S (book_not G (NVar (book_prop_name G)))"
  shows "book_lambda_I_derivable \<Sigma> G S (book_bottom G)"
proof -
  let ?p = "book_prop_name G"
  ...
  have replacement: "book_in_language book_minimal_logical_type UNIV \<Sigma> G ?T (G ?p)"
    by (simp only: ptype; rule conjunct1[OF tl])
  have free_for: "named_free_for ?T ?p (book_not G (NVar ?p))"
    by (simp only: book_lambda_I_not_variable_free_for_iff named_free_for.simps)
  have substituted: "book_lambda_I_derivable \<Sigma> G S
    (named_subst ?p ?T (book_not G (NVar ?p)))"
    by (rule book_lambda_I_variable_substitution[OF rich negative replacement conjunct2[OF tl] free_for])
  have negative_T: "book_lambda_I_derivable \<Sigma> G S (book_not G ?T)"
    using substituted by (simp add: book_lambda_I_not_variable_subst)
  have positive_T: "book_lambda_I_derivable \<Sigma> G S ?T"
    by (rule book_lambda_I_imp_refl[OF bottom])
  have unfolding_step: "book_lambda_I_derivable \<Sigma> G S
    (book_imp (book_not G ?T) (book_imp ?T (book_bottom G)))"
    by (rule book_lambda_I_not_unfold[OF rich tl])
  have conditional: "book_lambda_I_derivable \<Sigma> G S (book_imp ?T (book_bottom G))"
    by (rule book_lambda_I_derivable.MP[OF negative_T unfolding_step book_lambda_I_imp_language[OF tl bottom]])
  show ?thesis by (rule book_lambda_I_derivable.MP[OF positive_T conditional bottom])
qed

theorem book_lambda_I_open_propositional_decision:
  assumes rich: "sg_rich G"
    and decided: "book_lambda_I_derivable \<Sigma> G S (NVar (book_prop_name G)) \<or>
      book_lambda_I_derivable \<Sigma> G S (book_not G (NVar (book_prop_name G)))"
  shows "book_lambda_I_derivable \<Sigma> G S (book_bottom G)"
proof (rule disjE[OF decided])
  assume positive: "book_lambda_I_derivable \<Sigma> G S (NVar (book_prop_name G))"
  show ?thesis by (rule book_lambda_I_open_positive_bottom[OF rich positive])
next
  assume negative: "book_lambda_I_derivable \<Sigma> G S (book_not G (NVar (book_prop_name G)))"
  show ?thesis by (rule book_lambda_I_open_negative_bottom[OF rich negative])
qed
...
theorem book_lambda_I_all_open_decisions_bottom:
  assumes rich: "sg_rich G" and theory_ok: "book_lambda_I_higher_order_theory \<Sigma> G T"
    and complete: "\<And>A. book_lambda_I_formula \<Sigma> G A \<Longrightarrow> A \<in> T \<or> book_not G A \<in> T"
...
proof -
  ...
  have pl: "book_lambda_I_formula \<Sigma> G ?p"
    ...
    by (simp only: book_prop_name_type[OF rich] book_lambda_I.simps simp_thms)
  have npl: "book_lambda_I_formula \<Sigma> G (book_not G ?p)"
    by (rule book_lambda_I_not_language[OF rich pl])
  have decision: "book_lambda_I_derivable \<Sigma> G T ?p \<or>
      book_lambda_I_derivable \<Sigma> G T (book_not G ?p)"
    using complete[OF pl]
    by (blast intro: book_lambda_I_derivable.Assumption[OF _ pl]
        book_lambda_I_derivable.Assumption[OF _ npl])
  have contradiction: "book_lambda_I_derivable \<Sigma> G T (book_bottom G)"
    by (rule book_lambda_I_open_propositional_decision[OF rich decision])
  show ?thesis by (rule book_lambda_I_contains_derivation[OF theory_ok contradiction subset_refl])
qed
