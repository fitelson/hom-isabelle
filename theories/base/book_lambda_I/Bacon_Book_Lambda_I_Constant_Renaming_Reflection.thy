theory Bacon_Book_Lambda_I_Constant_Renaming_Reflection
  imports Bacon_Book_Lambda_I_Theory_Constant_Renaming Bacon_Book_Lambda_I_Theory_Consistency
begin
...
lemma book_lambda_I_constant_rename_inverse:
...
lemma book_lambda_I_constant_rename_inverse_image:
  assumes injective: "inj f"
  shows "image (book_constant_rename (inv f)) (image (book_constant_rename f) S) = S"
  by (simp add: image_image book_lambda_I_constant_rename_inverse[OF injective])

theorem book_lambda_I_constant_rename_iff:
  assumes injective: "inj f"
  shows "book_lambda_I_derivable \<Sigma> G S A \<longleftrightarrow>
    book_lambda_I_derivable (\<lambda>\<tau>. image f (\<Sigma> \<tau>)) G
      (image (book_constant_rename f) S) (book_constant_rename f A)"
proof
  assume original: "book_lambda_I_derivable \<Sigma> G S A"
  show "book_lambda_I_derivable (\<lambda>\<tau>. image f (\<Sigma> \<tau>)) G
      (image (book_constant_rename f) S) (book_constant_rename f A)"
    by (rule book_lambda_I_constant_rename_image[OF original])
next
  assume renamed: "book_lambda_I_derivable (\<lambda>\<tau>. image f (\<Sigma> \<tau>)) G
      (image (book_constant_rename f) S) (book_constant_rename f A)"
  ...
  qed
  have restored: "book_lambda_I_derivable \<Sigma> G
      (image (book_constant_rename (inv f)) (image (book_constant_rename f) S))
      (book_constant_rename (inv f) (book_constant_rename f A))"
    by (rule book_lambda_I_constant_rename[OF renamed inverse_maps])
  show "book_lambda_I_derivable \<Sigma> G S A"
    using restored by (simp only: book_lambda_I_constant_rename_inverse_image[OF injective]
      book_lambda_I_constant_rename_inverse[OF injective])
qed

theorem book_lambda_I_consistent_constant_rename_iff:
  assumes injective: "inj f"
  shows "book_lambda_I_consistent \<Sigma> G S \<longleftrightarrow>
    book_lambda_I_consistent (\<lambda>\<tau>. image f (\<Sigma> \<tau>)) G (image (book_constant_rename f) S)"
proof -
  have equivalence: "book_lambda_I_derivable \<Sigma> G S (book_bottom G) \<longleftrightarrow>
    book_lambda_I_derivable (\<lambda>\<tau>. image f (\<Sigma> \<tau>)) G
      (image (book_constant_rename f) S) (book_constant_rename f (book_bottom G))"
    by (rule book_lambda_I_constant_rename_iff[OF injective])
  show ?thesis unfolding book_lambda_I_consistent_def
...
corollary book_lambda_I_Inl_rename_iff:
  fixes \<Sigma> :: "'c ssignature"
  shows "book_lambda_I_derivable \<Sigma> G S A \<longleftrightarrow>
    book_lambda_I_derivable (\<lambda>\<tau>. image (Inl :: 'c \<Rightarrow> 'c + 'd) (\<Sigma> \<tau>)) G
      (image (book_constant_rename Inl) S) (book_constant_rename Inl A)"
  by (rule book_lambda_I_constant_rename_iff; rule inj_Inl)

corollary book_lambda_I_consistent_Inl_rename_iff:
  fixes \<Sigma> :: "'c ssignature"
  shows "book_lambda_I_consistent \<Sigma> G S \<longleftrightarrow>
    book_lambda_I_consistent (\<lambda>\<tau>. image (Inl :: 'c \<Rightarrow> 'c + 'd) (\<Sigma> \<tau>)) G
      (image (book_constant_rename Inl) S)"
  by (rule book_lambda_I_consistent_constant_rename_iff; rule inj_Inl)

lemma book_lambda_I_Inr_fresh_image_signature:
  "Inr u \<notin> image (Inl :: 'c \<Rightarrow> 'c + 'd) (\<Sigma> \<tau>)"
  by auto

corollary book_lambda_I_sum_image_has_fresh_name:
  fixes \<Sigma> :: "'c ssignature"
  shows "\<exists>c :: 'c + unit. \<forall>\<tau>. c \<notin> image Inl (\<Sigma> \<tau>)"
  by (rule exI[where x="Inr ()"]; rule allI; rule book_lambda_I_Inr_fresh_image_signature)
