theory Bacon_Book_Lambda_I_Closed_Witness_Completeness
...
text \<open>
  For each closed λI predicate F:σ→t, either ¬∃σF belongs to M or Fc
  belongs to M for some declared constant c:σ. This is the closed
  λI-predicate version of Bacon's witness completeness (Definition 15.4,
  p.319), strengthened from an arbitrary witness term to a declared
  constant.

  Representation. ∃σ remains the literal λ-defined operator. The
  predicate below tests only closed λI predicates, deliberately not
  unrestricted closed predicates; it does not itself
...
definition book_lambda_I_closed_constant_witness_complete ::
  "'c ssignature \<Rightarrow> sgcontext \<Rightarrow> 'c book_named_term set \<Rightarrow> bool" where
  "book_lambda_I_closed_constant_witness_complete \<Sigma> G M \<longleftrightarrow>
    (\<forall>\<sigma> F. book_in_language book_minimal_logical_type UNIV \<Sigma> G F (Arr \<sigma> Prop)
      \<longrightarrow> named_fv F = {} \<longrightarrow> book_lambda_I F \<longrightarrow>
...
text \<open>
  Suppose M is maximal among λI-consistent closed λI-formula extensions,
  and it contains W(F,c)=(∃σF)→Fc for some declared c for every closed
  λI predicate F.
...
theorem book_lambda_I_closed_constant_witness_complete_from_conditionals:
  assumes rich: "sg_rich G"
    ...
    and scheme: "\<And>\<sigma> F. book_in_language book_minimal_logical_type UNIV \<Sigma> G F (Arr \<sigma> Prop)
      \<Longrightarrow> named_fv F = {} \<Longrightarrow> book_lambda_I F \<Longrightarrow>
      \<exists>c\<in>\<Sigma> \<sigma>. book_witness_axiom G \<sigma> F c \<in> M"
  shows "book_lambda_I_closed_constant_witness_complete \<Sigma> G M"
proof (unfold book_lambda_I_closed_constant_witness_complete_def, intro allI impI)
  fix \<sigma> F
  ...
  have E_language: "book_lambda_I_formula \<Sigma> G ?E"
  ...
  proof (cases "?E \<in> M")
    ...
    obtain c where declared: "c \<in> \<Sigma> \<sigma>"
      ...
      using scheme[OF predicate closed lambda_I] by blast
    ...
    have instance_language: "book_lambda_I_formula \<Sigma> G (NApp F (NConst c \<sigma>))"
      by (rule conjI[OF book_language_App[OF predicate witness_constant]]; simp add: lambda_I)
    have instance_closed: "named_fv (NApp F (NConst c \<sigma>)) = {}"
      by (simp only: named_fv.simps closed Un_empty_right)
    have conditional_language: "book_lambda_I_formula \<Sigma> G (book_witness_axiom G \<sigma> F c)"
      by (rule book_lambda_I_witness_axiom_language[OF rich predicate declared lambda_I])
    have existential: "book_lambda_I_derivable \<Sigma> G M ?E"
      by (rule book_lambda_I_derivable.Assumption[OF True E_language])
    have conditional: "book_lambda_I_derivable \<Sigma> G M (book_witness_axiom G \<sigma> F c)"
      by (rule book_lambda_I_derivable.Assumption[OF conditional_member conditional_language])
    have implication: "book_lambda_I_derivable \<Sigma> G M (book_imp ?E (NApp F (NConst c \<sigma>)))"
      using conditional by (simp only: book_witness_axiom_def)
    have instance_derivation: "book_lambda_I_derivable \<Sigma> G M (NApp F (NConst c \<sigma>))"
      by (rule book_lambda_I_derivable.MP[OF existential implication instance_language])
    have instance_member: "NApp F (NConst c \<sigma>) \<in> M"
      by (rule book_lambda_I_closed_maximal_derivable_member[OF maximal instance_language instance_closed instance_derivation])
  ...
  next
    ...
    have decision: "?E \<in> M \<or> book_not G ?E \<in> M"
      by (rule book_lambda_I_closed_maximal_decides[OF rich maximal E_language E_closed])
