theory Bacon_Book_Lambda_I_Fresh_Constant_Generalization
  imports Bacon_Book_Lambda_I_Theory_Retraction Bacon_Book_Lambda_I_Theory_Variable_Substitution
    Bacon_Book_Environment_Development.Bacon_Book_Fresh_Constant_Generalization
begin
...
theorem book_lambda_I_fresh_constant_retraction:
  assumes rich: "sg_rich G"
    ...
    and derivation: "book_lambda_I_derivable \<Omega> G S (book_not G (NApp F (NConst c \<sigma>)))"
  shows "\<exists>x. G x = \<sigma> \<and> x \<notin> named_vars F \<and>
    book_lambda_I_derivable \<Sigma> G S (book_not G (NApp F (NVar x)))"
proof -
  let ?A = "book_not G (NApp F (NConst c \<sigma>))"
  obtain N where support: "book_lambda_I_retraction_support \<Sigma> G S ?A N"
    using book_lambda_I_derivable_retraction_support[where \<Omega>=\<Sigma>, OF derivation] by (elim exE)
  have finite_N: "finite N" by (rule book_lambda_I_retraction_support_finite[OF support])
  ...
  have retracted: "book_lambda_I_derivable \<Sigma> G (image (named_retract \<Sigma> ?v) S)
  ...
  have result: "book_lambda_I_derivable \<Sigma> G S (book_not G (NApp F (NVar (?v \<sigma>))))"
...
corollary book_lambda_I_fresh_constant_variable_instance:
  assumes rich: "sg_rich G"
    and fresh: "c \<notin> \<Sigma> \<sigma>"
    and premise_names: "\<And>A. A \<in> S \<Longrightarrow> named_in_signature \<Sigma> A"
    and predicate: "book_in_language book_minimal_logical_type UNIV \<Sigma> G F (Arr \<sigma> Prop)"
    and derivation: "book_lambda_I_derivable (book_add_constant \<Sigma> c \<sigma>) G S
      (book_not G (NApp F (NConst c \<sigma>)))"
  shows "\<exists>x. G x = \<sigma> \<and> x \<notin> named_vars F \<and>
    book_lambda_I_derivable \<Sigma> G S (book_not G (NApp F (NVar x)))"
  by (rule book_lambda_I_fresh_constant_retraction[where G=G and \<Sigma>=\<Sigma> and \<sigma>=\<sigma>
...
theorem book_lambda_I_fresh_constant_generalization:
  assumes rich: "sg_rich G"
    and fresh: "c \<notin> \<Sigma> \<sigma>"
    and premise_names: "\<And>A. A \<in> S \<Longrightarrow> named_in_signature \<Sigma> A"
    and predicate: "book_in_language book_minimal_logical_type UNIV \<Sigma> G F (Arr \<sigma> Prop)"
    and derivation: "book_lambda_I_derivable (book_add_constant \<Sigma> c \<sigma>) G S
      (book_not G (NApp F (NConst c \<sigma>)))"
  shows "\<exists>x. G x = \<sigma> \<and> x \<notin> named_vars F \<and>
    book_lambda_I_derivable \<Sigma> G S (book_all G x (book_not G (NApp F (NVar x))))"
proof -
  obtain x where xtype: "G x = \<sigma>" and xfresh: "x \<notin> named_vars F"
    and variable_instance: "book_lambda_I_derivable \<Sigma> G S (book_not G (NApp F (NVar x)))"
    using book_lambda_I_fresh_constant_variable_instance[where G=G and \<Sigma>=\<Sigma> and \<sigma>=\<sigma>
      and c=c and F=F,
      OF rich fresh premise_names predicate derivation] by blast
  have generalized: "book_lambda_I_derivable \<Sigma> G S
    (book_all G x (book_not G (NApp F (NVar x))))"
    by (rule book_lambda_I_generalize[OF rich variable_instance]; unfold book_not_def; simp)
