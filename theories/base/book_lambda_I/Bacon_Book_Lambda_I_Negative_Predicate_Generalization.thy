theory Bacon_Book_Lambda_I_Negative_Predicate_Generalization
  imports Bacon_Book_Lambda_I_Open_Propositional_Decision Bacon_Book_Environment_Development.Bacon_Book_Minimal_Existential_Syntax
begin
...
text \<open>
  ...
  Definition 5.1, pp.97–98, and Definition 5.2, p.99. The λI rules used
...
lemma book_lambda_I_negative_predicate_free_for:
...
proof -
  ...
  show ?thesis by (simp only: book_lambda_I_not_variable_free_for_iff named_free_for.simps head_free; simp)
qed

lemma book_lambda_I_negative_predicate_subst:
  assumes fresh: "x \<notin> named_fv F"
...
proof -
  ...
  show ?thesis by (simp add: book_lambda_I_not_variable_subst head_fixed)
qed

theorem book_lambda_I_negative_predicate_rename:
  assumes rich: "sg_rich G"
    and negative: "book_lambda_I_derivable \<Sigma> G S (book_not G (NApp F (NVar x)))"
  ...
  shows "book_lambda_I_derivable \<Sigma> G S (book_not G (NApp F (NVar y)))"
proof -
  ...
  have free_for: "named_free_for (NVar y) x (book_not G (NApp F (NVar x)))"
    by (rule book_lambda_I_negative_predicate_free_for[OF fresh])
  have replacement_lambda_I: "book_lambda_I (NVar y)" by simp
  have substituted: "book_lambda_I_derivable \<Sigma> G S
    (named_subst x (NVar y) (book_not G (NApp F (NVar x))))"
    by (rule book_lambda_I_variable_substitution[OF rich negative replacement replacement_lambda_I free_for])
  show ?thesis using substituted by (simp only: book_lambda_I_negative_predicate_subst[OF fresh])
qed
...
theorem book_lambda_I_negative_predicate_generalize:
  assumes rich: "sg_rich G"
    and negative: "book_lambda_I_derivable \<Sigma> G S (book_not G (NApp F (NVar x)))"
    and predicate: "book_in_language book_minimal_logical_type UNIV \<Sigma> G F (Arr \<sigma> Prop)"
    and xtype: "G x = \<sigma>"
...
proof -
  ...
  have moved: "book_lambda_I_derivable \<Sigma> G S (book_not G (NApp F (NVar ?y)))"
    by (rule book_lambda_I_negative_predicate_rename[OF rich negative predicate xtype ytype fresh])
  have occurs: "?y \<in> named_fv (book_not G (NApp F (NVar ?y)))" unfolding book_not_def by simp
  show ?thesis by (rule book_lambda_I_generalize[OF rich moved occurs])
qed
