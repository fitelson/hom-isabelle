theory Bacon_Book_Lambda_I_Quantifier_Proof_Basics
  imports Bacon_Book_Lambda_I_Theory_Consistency Bacon_Book_Lambda_I_Existential_Conversion
begin
...
theorem book_lambda_I_double_negation_schema:
  assumes rich: "sg_rich G" and al: "book_lambda_I_formula \<Sigma> G A"
  shows "book_lambda_I_derivable \<Sigma> G {} (book_imp (book_not G (book_not G A)) A)"
proof -
  have nal: "book_lambda_I_formula \<Sigma> G (book_not G A)"
    by (rule book_lambda_I_not_language[OF rich al])
  have nnal: "book_lambda_I_formula \<Sigma> G (book_not G (book_not G A))"
    by (rule book_lambda_I_not_language[OF rich nal])
  have expanded: "book_lambda_I_formula \<Sigma> G (book_imp (book_not G A) (book_bottom G))"
    by (rule book_lambda_I_imp_language[OF nal book_lambda_I_bottom_language[OF rich]])
  have unfolding_step: "book_lambda_I_derivable \<Sigma> G {}
    (book_imp (book_not G (book_not G A)) (book_imp (book_not G A) (book_bottom G)))"
    by (rule book_lambda_I_not_unfold[OF rich nal])
  have refutation: "book_lambda_I_derivable \<Sigma> G {}
    (book_imp (book_imp (book_not G A) (book_bottom G)) A)"
    by (rule book_lambda_I_refutation_schema[OF rich al])
  show ?thesis by (rule book_lambda_I_imp_trans[OF unfolding_step refutation nnal expanded al])
qed

corollary book_lambda_I_double_negation_elim:
  assumes rich: "sg_rich G" and al: "book_lambda_I_formula \<Sigma> G A"
    and negative: "book_lambda_I_derivable \<Sigma> G S (book_not G (book_not G A))"
  shows "book_lambda_I_derivable \<Sigma> G S A"
proof -
  have schema: "book_lambda_I_derivable \<Sigma> G {} (book_imp (book_not G (book_not G A)) A)"
    by (rule book_lambda_I_double_negation_schema[OF rich al])
  have lifted: "book_lambda_I_derivable \<Sigma> G S (book_imp (book_not G (book_not G A)) A)"
    by (rule book_lambda_I_derivable_mono[OF schema empty_subsetI])
  show ?thesis by (rule book_lambda_I_derivable.MP[OF negative lifted al])
qed
...
theorem book_lambda_I_forall_eta_pair:
...
proof -
  ...
  have body_language: "book_lambda_I_formula \<Sigma> G (NApp F (NVar ?y))"
    by (rule conjI[OF book_language_App[OF predicate variable]]; simp add: lambda_I)
  have occurs: "?y \<in> named_fv (NApp F (NVar ?y))" by simp
  have binder_language: "book_lambda_I_formula \<Sigma> G ?B"
    by (rule book_lambda_I_all_language[OF body_language occurs])
  have primitive_language: "book_lambda_I_formula \<Sigma> G ?Q"
  ...
  have forward: "book_lambda_I_derivable \<Sigma> G S (book_imp ?Q ?B)"
    by (rule book_lambda_I_derivable.Eta[OF primitive_language binder_language],
      rule disjI2, rule contraction)
  have backward: "book_lambda_I_derivable \<Sigma> G S (book_imp ?B ?Q)"
    by (rule book_lambda_I_derivable.Eta[OF binder_language primitive_language],
...
text \<open>
  ...
  closed because F is closed. This route is carried out in
  Bacon_Book_Lambda_I_Closed_Quantifier_Duality
  (book_lambda_I_closed_forall_dual); it is not asserted here.
\<close>
