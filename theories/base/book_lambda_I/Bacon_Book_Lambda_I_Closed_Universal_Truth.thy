theory Bacon_Book_Lambda_I_Closed_Universal_Truth
  imports Bacon_Book_Lambda_I_Closed_Universal_Instances Bacon_Book_Lambda_I_Closed_Quantifier_Duality Bacon_Book_Lambda_I_Closed_Witness_Completeness
begin

section \<open>Applying the literal negative predicate to a closed argument\<close>

lemma book_lambda_I_negated_predicate_closed_application_beta:
  assumes predicate_closed: "named_fv F = {}"
    and argument_closed: "named_fv A = {}"
  shows "named_beta_contract (NApp (book_lambda_I_negated_predicate G \<sigma> F) A)
    (book_not G (NApp F A))"
proof -
  let ?y = "book_exists_argument_name G \<sigma>"
  let ?B = "book_not G (NApp F (NVar ?y))"
  have free_for: "named_free_for A ?y ?B"
    by (rule book_lambda_I_exists_closed_free_for[OF argument_closed])
  ...
  show ?thesis using raw by (simp only: book_lambda_I_negated_predicate_def substitution)
qed

section \<open>Closed universal membership and all closed instances\<close>

text \<open>
  For a closed λI predicate F:σ→t, ∀σF∈M iff FA∈M for every closed λI
  term A:σ of Σ, provided M is maximal closed λI-consistent and has
  closed constant witnesses for closed λI predicates.
...
theorem book_lambda_I_closed_maximal_forall_iff:
  ...
  shows "NApp (NLogical (SBAll \<sigma>)) F \<in> M \<longleftrightarrow>
    (\<forall>A\<in>book_lambda_I_closed_terms \<Sigma> G \<sigma>. NApp F A \<in> M)"
proof -
  ...
  have fc: "named_fv F = {}" by (rule book_lambda_I_closed_terms_closed[OF predicate])
  ...
  proof
    ...
    show "\<forall>A\<in>book_lambda_I_closed_terms \<Sigma> G \<sigma>. NApp F A \<in> M"
    proof (rule ballI)
      fix A
      assume argument: "A \<in> book_lambda_I_closed_terms \<Sigma> G \<sigma>"
      show "NApp F A \<in> M"
        by (rule book_lambda_I_closed_maximal_forall_instance[OF maximal predicate argument universal])
    qed
  next
    assume instances: "\<forall>A\<in>book_lambda_I_closed_terms \<Sigma> G \<sigma>. NApp F A \<in> M"
    ...
    have ql: "book_in_language book_minimal_logical_type UNIV \<Sigma> G ?Q (Arr \<sigma> Prop)"
      by (rule book_lambda_I_negated_predicate_language[OF rich fl fr])
    have qc: "named_fv ?Q = {}" by (rule book_lambda_I_negated_predicate_closed[OF fc])
    have qr: "book_lambda_I ?Q" by (rule book_lambda_I_negated_predicate_relevant[OF fr])
    have choice: "book_not G ?E \<in> M \<or> (\<exists>c\<in>\<Sigma> \<sigma>. NApp ?Q (NConst c \<sigma>) \<in> M)"
      using witnesses ql qc qr unfolding book_lambda_I_closed_constant_witness_complete_def by blast
    show "?U \<in> M"
    proof (rule disjE[OF choice])
      assume negative_member: "book_not G ?E \<in> M"
      have negative_language: "book_lambda_I_formula \<Sigma> G (book_not G ?E)"
        by (rule book_lambda_I_not_language[OF rich book_lambda_I_exists_application_language[OF rich ql qr]])
      have negative: "book_lambda_I_derivable \<Sigma> G M (book_not G ?E)"
        by (rule book_lambda_I_derivable.Assumption[OF negative_member negative_language])
      have ul: "book_lambda_I_formula \<Sigma> G ?U"
        by (rule conjI[OF book_language_App[OF book_all_operator_language fl]]; simp add: fr)
      have uc: "named_fv ?U = {}" by (simp add: fc)
      have dual: "book_lambda_I_derivable \<Sigma> G {} (book_imp (book_not G ?E) ?U)"
        by (rule book_lambda_I_closed_forall_dual[OF rich fl fc fr])
      have lifted: "book_lambda_I_derivable \<Sigma> G M (book_imp (book_not G ?E) ?U)"
        by (rule book_lambda_I_derivable_mono[OF dual empty_subsetI])
      have derived: "book_lambda_I_derivable \<Sigma> G M ?U"
        by (rule book_lambda_I_derivable.MP[OF negative lifted ul])
      show ?thesis by (rule book_lambda_I_closed_maximal_derivable_member[OF maximal ul uc derived])
    next
      ...
      have argument: "?a \<in> book_lambda_I_closed_terms \<Sigma> G \<sigma>"
        by (rule book_lambda_I_closed_termsI[OF al ac]; simp)
      ...
      have instance_language: "book_lambda_I_formula \<Sigma> G (NApp F ?a)"
        by (rule conjI[OF book_language_App[OF fl al]]; simp add: fr)
      have instance_closed: "named_fv (NApp F ?a) = {}" by (simp add: fc)
      have negative_language: "book_lambda_I_formula \<Sigma> G (book_not G (NApp F ?a))"
        by (rule book_lambda_I_not_language[OF rich instance_language])
      have negative_closed: "named_fv (book_not G (NApp F ?a)) = {}"
        by (simp only: book_not_fv instance_closed)
      have redex_language: "book_lambda_I_formula \<Sigma> G (NApp ?Q ?a)"
        by (rule conjI[OF book_language_App[OF ql al]]; simp add: qr)
      have redex: "book_lambda_I_derivable \<Sigma> G M (NApp ?Q ?a)"
        by (rule book_lambda_I_derivable.Assumption[OF member redex_language])
      have contraction: "named_beta_contract (NApp ?Q ?a) (book_not G (NApp F ?a))"
        by (rule book_lambda_I_negated_predicate_closed_application_beta[OF fc ac])
      have beta: "book_lambda_I_derivable \<Sigma> G M
        (book_imp (NApp ?Q ?a) (book_not G (NApp F ?a)))"
        by (rule book_lambda_I_derivable.Beta[OF redex_language negative_language],
          rule disjI1, rule named_compatible_step.root, rule contraction)
      have negative: "book_lambda_I_derivable \<Sigma> G M (book_not G (NApp F ?a))"
        by (rule book_lambda_I_derivable.MP[OF redex beta negative_language])
      have negative_member: "book_not G (NApp F ?a) \<in> M"
        by (rule book_lambda_I_closed_maximal_derivable_member[OF maximal negative_language negative_closed negative])
      have absent: "NApp F ?a \<notin> M"
        by (rule iffD1[OF book_lambda_I_closed_maximal_negation_iff[
