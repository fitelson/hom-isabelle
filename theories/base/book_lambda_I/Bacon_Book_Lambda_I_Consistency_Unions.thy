theory Bacon_Book_Lambda_I_Consistency_Unions
  imports Bacon_Book_Lambda_I_Theory_Consistency
begin
...
lemma book_lambda_I_finite_directed_cover:
...
theorem book_lambda_I_consistent_directed_Union:
  assumes nonempty: "\<C> \<noteq> {}"
    and directed: "\<And>U V. U \<in> \<C> \<Longrightarrow> V \<in> \<C> \<Longrightarrow>
      \<exists>W\<in>\<C>. U \<union> V \<subseteq> W"
    and consistent: "\<And>U. U \<in> \<C> \<Longrightarrow> book_lambda_I_consistent \<Sigma> G U"
  shows "book_lambda_I_consistent \<Sigma> G (\<Union>\<C>)"
proof (rule iffD2[OF book_lambda_I_consistent_finite_iff], intro allI impI)
  fix F
  assume finite_F: "finite F" and included: "F \<subseteq> \<Union>\<C>"
  obtain U where member: "U \<in> \<C>" and covered: "F \<subseteq> U"
    using book_lambda_I_finite_directed_cover[OF finite_F included nonempty directed] by blast
  show "book_lambda_I_consistent \<Sigma> G F"
    by (rule book_lambda_I_consistent_subset[OF consistent[OF member] covered])
qed

corollary book_lambda_I_consistent_chain_Union:
  assumes nonempty: "\<C> \<noteq> {}"
    and chain: "\<And>U V. U \<in> \<C> \<Longrightarrow> V \<in> \<C> \<Longrightarrow> U \<subseteq> V \<or> V \<subseteq> U"
    and consistent: "\<And>U. U \<in> \<C> \<Longrightarrow> book_lambda_I_consistent \<Sigma> G U"
  shows "book_lambda_I_consistent \<Sigma> G (\<Union>\<C>)"
proof (rule book_lambda_I_consistent_directed_Union[OF nonempty _ consistent])
  fix U V
