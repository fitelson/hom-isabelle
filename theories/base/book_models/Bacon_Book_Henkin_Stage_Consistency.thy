theory Bacon_Book_Henkin_Stage_Consistency
  imports Bacon_Book_Henkin_Stage_Witnesses Bacon_Book_Infinite_Witness_Family
begin

section \<open>All closed-predicate witnesses at one stage preserve consistency\<close>

text \<open>
  At stage n, use the ACTUAL family of all closed predicates of Σₙ,
...
theorem book_henkin_stage_consistent:
  assumes rich: "sg_rich G"
    and consistent: "book_theory_consistent (book_henkin_signature \<Sigma> G n) G S"
    and language: "\<And>A. A \<in> S \<Longrightarrow>
      book_theory_formula (book_henkin_signature \<Sigma> G n) G A"
  shows "book_theory_consistent (book_henkin_signature \<Sigma> G (Suc n)) G
...
proof -
  ...
  have family: "book_theory_consistent
      (book_witness_family_signature (book_henkin_signature \<Sigma> G n) fst ?c ?I) G
      (S \<union> book_witness_family_axioms G fst snd ?c ?I)"
    by (rule book_theory_consistent_witness_family[where \<Sigma>="book_henkin_signature \<Sigma> G n"
