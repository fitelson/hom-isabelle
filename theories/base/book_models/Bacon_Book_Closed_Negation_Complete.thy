theory Bacon_Book_Closed_Negation_Complete
  imports Bacon_Book_Closed_Maximal_Extension
begin
...
lemma book_theory_consistent_insert_consequence:
  assumes consistent: "book_theory_consistent \<Sigma> G S"
    and consequence: "book_theory_derivable \<Sigma> G S A"
    and language: "\<And>B. B \<in> S \<Longrightarrow> book_theory_formula \<Sigma> G B"
  shows "book_theory_consistent \<Sigma> G (insert A S)"
proof (unfold book_theory_consistent_def, rule notI)
  assume contradiction: "book_theory_derivable \<Sigma> G (insert A S) (book_bottom G)"
  have original: "book_theory_derivable \<Sigma> G S (book_bottom G)"
  proof (rule book_theory_derivable_cut[OF contradiction])
    fix B
    ...
    next
      ...
      show ?thesis by (rule book_theory_derivable.Assumption[OF old_member language[OF old_member]])
    qed
  qed
  show False using consistent original unfolding book_theory_consistent_def by blast
qed
...
proof -
  ...
  have language: "book_theory_formula \<Sigma> G B" if "B \<in> M" for B
    by (rule conjunct1[OF book_closed_formula_set_member[OF closed_M that]])
  have consistent_insert: "book_theory_consistent \<Sigma> G (insert A M)"
    by (rule book_theory_consistent_insert_consequence[OF consistent_M derivation language])
  have closed_insert: "book_closed_formula_set \<Sigma> G (insert A M)"
    using closed_M al closed unfolding book_closed_formula_set_def by auto
...
text \<open>
  For each typed closed A, either A∈M or ¬A∈M. If A∉M then
  closed-consequence closure gives M ⊬ A. The repaired consistency
  corollary makes M∪{¬A} consistent, and maximality puts ¬A in M.
...
theorem book_closed_maximal_decides:
...
next
  case False
  have nonderivable: "\<not> book_theory_derivable \<Sigma> G M A"
  proof
    assume derivation: "book_theory_derivable \<Sigma> G M A"
    have member: "A \<in> M"
      by (rule book_closed_maximal_derivable_member[OF maximal al closed derivation])
  ...
  have closed_insert: "book_closed_formula_set \<Sigma> G (insert (book_not G A) M)"
    using closed_M nal negative_closed unfolding book_closed_formula_set_def by auto
  have consistent_insert: "book_theory_consistent \<Sigma> G (insert (book_not G A) M)"
    by (rule book_theory_consistent_insert_not[OF rich al closed nonderivable])
...
theorem book_closed_maximal_negation_iff:
  assumes rich: "sg_rich G"
    and maximal: "book_closed_maximal_extension \<Sigma> G S M"
    and al: "book_theory_formula \<Sigma> G A" and closed: "named_fv A = {}"
...
proof
  ...
  proof
    ...
    have nal: "book_theory_formula \<Sigma> G (book_not G A)"
      by (rule book_not_language[OF rich al])
    have positive: "book_prop_certificate \<Sigma> G M A"
      by (rule book_prop_certificate.Assumption[OF positive_member al])
    have negative: "book_prop_certificate \<Sigma> G M (book_not G A)"
      by (rule book_prop_certificate.Assumption[OF negative_member nal])
    have bottom_certificate: "book_prop_certificate \<Sigma> G M (book_bottom G)"
      by (rule book_prop_certificate_not_elim[OF rich positive negative])
    have contradiction: "book_theory_derivable \<Sigma> G M (book_bottom G)"
      by (rule book_prop_certificate_embeds[OF bottom_certificate])
    have consistent_M: "book_theory_consistent \<Sigma> G M"
      using maximal unfolding book_closed_maximal_extension_def by blast
...
next
  ...
  have decision: "A \<in> M \<or> book_not G A \<in> M"
    by (rule book_closed_maximal_decides[OF rich maximal al closed])
  show "book_not G A \<in> M" using decision absent by blast
qed

corollary book_closed_decision_extension_exists:
  assumes rich: "sg_rich G" and closed_S: "book_closed_formula_set \<Sigma> G S"
    and consistent_S: "book_theory_consistent \<Sigma> G S"
  shows "\<exists>M. book_closed_maximal_extension \<Sigma> G S M \<and>
    (\<forall>A. book_theory_formula \<Sigma> G A \<longrightarrow> named_fv A = {} \<longrightarrow>
      A \<in> M \<or> book_not G A \<in> M)"
proof -
  obtain M where maximal: "book_closed_maximal_extension \<Sigma> G S M"
    using book_closed_maximal_extension_exists[OF closed_S consistent_S] by blast
  have decisions: "\<forall>A. book_theory_formula \<Sigma> G A \<longrightarrow> named_fv A = {} \<longrightarrow>
      A \<in> M \<or> book_not G A \<in> M"
    by (intro allI impI; rule book_closed_maximal_decides[OF rich maximal]; assumption)
