theory Bacon_Book_Negation_Conversion
...
text \<open>
  ...
  printed β schema gives both ⊢ ¬A→(A→⊥) and ⊢ (A→⊥)→¬A.
...
theorem book_theory_not_unfold:
  assumes rich: "sg_rich G" and formula: "book_theory_formula \<Sigma> G A"
  shows "book_theory_derivable \<Sigma> G S
    (book_imp (book_not G A) (book_imp A (book_bottom G)))"
proof -
  have negated: "book_theory_formula \<Sigma> G (book_not G A)"
    by (rule book_not_language[OF rich formula])
  have expanded: "book_theory_formula \<Sigma> G (book_imp A (book_bottom G))"
    by (rule book_imp_language[OF formula book_bottom_language[OF rich]])
  show ?thesis by (rule book_theory_derivable.Beta[OF negated expanded],
      rule disjI1, rule book_not_beta_step)
qed

theorem book_theory_not_fold:
  assumes rich: "sg_rich G" and formula: "book_theory_formula \<Sigma> G A"
  shows "book_theory_derivable \<Sigma> G S
...
proof -
  ...
  have expanded: "book_theory_formula \<Sigma> G (book_imp A (book_bottom G))"
    by (rule book_imp_language[OF formula book_bottom_language[OF rich]])
  show ?thesis by (rule book_theory_derivable.Beta[OF expanded negated],
      rule disjI2, rule book_not_beta_step)
qed

corollary book_theory_not_conversion_pair:
  assumes rich: "sg_rich G" and formula: "book_theory_formula \<Sigma> G A"
  shows "book_theory_derivable \<Sigma> G S (book_imp (book_not G A) (book_imp A (book_bottom G))) \<and>
    book_theory_derivable \<Sigma> G S (book_imp (book_imp A (book_bottom G)) (book_not G A))"
  by (rule conjI[OF book_theory_not_unfold[OF rich formula] book_theory_not_fold[OF rich formula]])
...
theorem book_theory_not_bottom:
  assumes rich: "sg_rich G"
  shows "book_theory_derivable \<Sigma> G S (book_not G (book_bottom G))"
proof -
  have bottom: "book_theory_formula \<Sigma> G (book_bottom G)"
    by (rule book_bottom_language[OF rich])
  have reflexive: "book_theory_derivable \<Sigma> G S (book_imp (book_bottom G) (book_bottom G))"
    by (rule book_theory_imp_refl[OF bottom])
  have folded: "book_theory_derivable \<Sigma> G S
    (book_imp (book_imp (book_bottom G) (book_bottom G)) (book_not G (book_bottom G)))"
    by (rule book_theory_not_fold[OF rich bottom])
  show ?thesis by (rule book_theory_derivable.MP[OF reflexive folded book_not_language[OF rich bottom]])
qed
