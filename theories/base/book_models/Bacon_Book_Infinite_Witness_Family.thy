theory Bacon_Book_Infinite_Witness_Family
  imports Bacon_Book_Finite_Witness_Family Bacon_Book_Finite_Image_Cover
begin

section \<open>Arbitrary families of fresh conditional witness premises\<close>

text \<open>
  Let every Fᵢ:τᵢ→t be closed and belong to the OLD signature Σ.
  If the names cᵢ are injective on I and fresh at their indicated types,
  consistency of S implies consistency of S∪W[I] in Σ[I].
...
theorem book_theory_consistent_witness_family:
...
proof (rule iffD2[OF book_theory_consistent_finite_iff])
  ...
  show "\<forall>B. finite B \<longrightarrow> B \<subseteq> S \<union> book_witness_family_axioms G \<tau> F c I
    \<longrightarrow> book_theory_consistent ?full G B"
  proof (intro allI impI)
    ...
    have partial_consistent: "book_theory_consistent ?partial G ?premises"
      by (rule book_theory_consistent_finite_witness_family[where \<Sigma>=\<Sigma> and G=G
        and S=S and \<tau>=\<tau> and F=F and c=c and I=J, OF finite_J rich consistent
        language predicates_J closed_J fresh_J injective_J])
    have partial_names: "named_in_signature ?partial A" if "A \<in> ?premises" for A
    proof -
      have premise_member: "A \<in> ?premises" by (rule that)
      have premise_language: "book_theory_formula ?partial G A"
        by (rule book_witness_family_premises_language[OF rich language predicates_J premise_member])
      show "named_in_signature ?partial A" by (rule book_language_signature[OF premise_language])
    qed
    have full_consistent: "book_theory_consistent ?full G ?premises"
      by (rule book_theory_consistent_signature_transport[OF rich partial_consistent partial_names])
    have selected_subset: "B \<subseteq> ?premises"
      using covered_J by (simp only: book_witness_family_axioms_def)
    show "book_theory_consistent ?full G B"
      by (rule book_theory_consistent_subset[OF full_consistent selected_subset])
  qed
qed
