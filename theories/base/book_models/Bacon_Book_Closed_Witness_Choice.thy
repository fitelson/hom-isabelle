theory Bacon_Book_Closed_Witness_Choice
  imports Bacon_Book_Fresh_Constant_Generalization Bacon_Book_Existential_Conversion
    Bacon_Book_Negative_Predicate_Generalization Bacon_Book_Theory_Consistency
    Bacon_Book_Theory_Signature_Conservativity
begin

section \<open>Failure of a closed positive extension yields its negation\<close>

lemma book_theory_closed_inconsistency_not:
  assumes rich: "sg_rich G" and al: "book_theory_formula \<Sigma> G A"
    and closed: "named_fv A = {}"
    and inconsistent: "\<not> book_theory_consistent \<Sigma> G (insert A S)"
  shows "book_theory_derivable \<Sigma> G S (book_not G A)"
proof -
  have contradiction: "book_theory_derivable \<Sigma> G (insert A S) (book_bottom G)"
    using inconsistent unfolding book_theory_consistent_def by blast
  have conditional: "book_theory_derivable \<Sigma> G S (book_imp A (book_bottom G))"
    by (rule book_theory_closed_deduction[OF rich al closed contradiction])
  have folding: "book_theory_derivable \<Sigma> G S
    (book_imp (book_imp A (book_bottom G)) (book_not G A))"
    by (rule book_theory_not_fold[OF rich al])
  show ?thesis by (rule book_theory_derivable.MP[OF conditional folding book_not_language[OF rich al]])
qed
...
theorem book_theory_closed_witness_choice:
  ...
  shows "book_theory_consistent (book_add_constant \<Sigma> c \<sigma>) G (insert (NApp F (NConst c \<sigma>)) S) \<or>
    book_theory_consistent (book_add_constant \<Sigma> c \<sigma>) G
...
proof (rule ccontr)
  ...
  assume neither: "\<not> (book_theory_consistent ?\<Omega> G (insert ?Fc S) \<or>
      book_theory_consistent ?\<Omega> G (insert (book_not G ?E) S))"
  have first_bad: "\<not> book_theory_consistent ?\<Omega> G (insert ?Fc S)"
    and second_bad: "\<not> book_theory_consistent ?\<Omega> G (insert (book_not G ?E) S)"
  ...
  have Fc_language: "book_theory_formula ?\<Omega> G ?Fc"
    by (rule book_language_App[OF enlarged_F witness_constant])
  have Fc_closed: "named_fv ?Fc = {}" using closed by simp
  have negative_c: "book_theory_derivable ?\<Omega> G S (book_not G ?Fc)"
    by (rule book_theory_closed_inconsistency_not[OF rich Fc_language Fc_closed first_bad])
  obtain x where xtype: "G x = \<sigma>" and xnames: "x \<notin> named_vars F"
    and negative_x: "book_theory_derivable \<Sigma> G S (book_not G (NApp F (NVar x)))"
    using book_theory_fresh_constant_retraction[where G=G and \<Sigma>=\<Sigma> and \<sigma>=\<sigma>
  ...
  have universal_negative: "book_theory_derivable \<Sigma> G S ?P"
    by (rule book_theory_negative_predicate_generalize[OF rich negative_x predicate xtype xfree])
  have enlarged_E: "book_theory_formula ?\<Omega> G ?E"
  ...
  have positive_enlarged: "book_theory_derivable ?\<Omega> G S ?E"
    by (rule book_theory_closed_inconsistency_refutation[OF rich enlarged_E E_closed second_bad])
  have old_E: "book_theory_formula \<Sigma> G ?E"
    by (rule book_exists_application_language[OF rich predicate])
  have old_E_names: "named_in_signature \<Sigma> ?E" by (rule book_language_signature[OF old_E])
  have positive: "book_theory_derivable \<Sigma> G S ?E"
    by (rule book_theory_foreign_constants_eliminate[OF rich positive_enlarged old_E_names];
  ...
  have P_language: "book_theory_formula \<Sigma> G ?P"
    by (rule book_all_language[OF book_not_language[OF rich book_language_App[OF predicate y_language]]])
  have unfolding_step: "book_theory_derivable \<Sigma> G S (book_imp ?E (book_not G ?P))"
    by (rule book_theory_exists_unfold[OF rich predicate closed])
  have negative_P: "book_theory_derivable \<Sigma> G S (book_not G ?P)"
    by (rule book_theory_derivable.MP[OF positive unfolding_step book_not_language[OF rich P_language]])
  have refutation: "book_theory_derivable \<Sigma> G S
    (book_imp (book_not G ?P) (book_imp ?P (book_bottom G)))"
    by (rule book_theory_not_unfold[OF rich P_language])
  have conditional: "book_theory_derivable \<Sigma> G S (book_imp ?P (book_bottom G))"
    by (rule book_theory_derivable.MP[OF negative_P refutation
      book_imp_language[OF P_language book_bottom_language[OF rich]]])
  have contradiction: "book_theory_derivable \<Sigma> G S (book_bottom G)"
    by (rule book_theory_derivable.MP[OF universal_negative conditional book_bottom_language[OF rich]])
  show False using consistent contradiction unfolding book_theory_consistent_def by blast
qed
