theory Bacon_Book_Existential_Conversion
  imports Bacon_Book_Minimal_Existential_Syntax Bacon_Book_Theory_Conversion
begin
...
lemma book_exists_beta_contract:
...
proof -
  ...
  have free_for: "named_free_for F ?X ?body" by (rule book_exists_closed_free_for[OF closed])
  ...
  show ?thesis using contraction
    by (simp only: book_exists_const_as_all[OF rich] book_exists_body_subst[OF rich])
qed

lemma book_exists_beta_step:
  assumes rich: "sg_rich G" and closed: "named_fv F = {}"
  shows "named_compatible_step named_beta_contract (NApp (book_exists_const G \<sigma>) F)
    (book_not G (book_all G (book_exists_argument_name G \<sigma>)
      (book_not G (NApp F (NVar (book_exists_argument_name G \<sigma>))))))"
  by (rule named_compatible_step.root; rule book_exists_beta_contract[OF rich closed])

lemma book_exists_expansion_language:
  assumes rich: "sg_rich G"
    and predicate: "book_in_language book_minimal_logical_type UNIV \<Sigma> G F (Arr \<sigma> Prop)"
  shows "book_theory_formula \<Sigma> G
    (book_not G (book_all G (book_exists_argument_name G \<sigma>)
      (book_not G (NApp F (NVar (book_exists_argument_name G \<sigma>))))))"
proof -
  ...
  have application: "book_theory_formula \<Sigma> G (NApp F (NVar (book_exists_argument_name G \<sigma>)))"
    by (rule book_language_App[OF predicate variable])
  show ?thesis by (rule book_not_language[OF rich book_all_language[OF book_not_language[OF rich application]]])
qed
...
theorem book_theory_exists_unfold:
  assumes rich: "sg_rich G"
    and predicate: "book_in_language book_minimal_logical_type UNIV \<Sigma> G F (Arr \<sigma> Prop)"
  ...
  shows "book_theory_derivable \<Sigma> G S
    (book_imp (NApp (book_exists_const G \<sigma>) F)
      (book_not G (book_all G (book_exists_argument_name G \<sigma>)
        (book_not G (NApp F (NVar (book_exists_argument_name G \<sigma>)))))))"
  by (rule book_theory_derivable.Beta[
      OF book_exists_application_language[OF rich predicate] book_exists_expansion_language[OF rich predicate]],
      rule disjI1, rule book_exists_beta_step[OF rich closed])

theorem book_theory_exists_fold:
  assumes rich: "sg_rich G"
    and predicate: "book_in_language book_minimal_logical_type UNIV \<Sigma> G F (Arr \<sigma> Prop)"
    and closed: "named_fv F = {}"
  shows "book_theory_derivable \<Sigma> G S
  ...
  by (rule book_theory_derivable.Beta[
      OF book_exists_expansion_language[OF rich predicate] book_exists_application_language[OF rich predicate]],
      rule disjI2, rule book_exists_beta_step[OF rich closed])

theorem book_theory_exists_conversion_pair:
  assumes rich: "sg_rich G"
    and predicate: "book_in_language book_minimal_logical_type UNIV \<Sigma> G F (Arr \<sigma> Prop)"
    and closed: "named_fv F = {}"
  shows "book_theory_derivable \<Sigma> G S
    ...
    book_theory_derivable \<Sigma> G S
      (book_imp (book_not G (book_all G (book_exists_argument_name G \<sigma>)
          (book_not G (NApp F (NVar (book_exists_argument_name G \<sigma>))))))
        (NApp (book_exists_const G \<sigma>) F))"
  by (rule conjI[OF book_theory_exists_unfold[OF rich predicate closed]
      book_theory_exists_fold[OF rich predicate closed]])
